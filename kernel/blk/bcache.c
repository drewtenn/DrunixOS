...
static void lru_detach(bcache_slot_t *s)
{
    ...
    else         g_lru_head    = s->next;
    if (s->next) s->next->prev = s->prev;
    else         g_lru_tail    = s->prev;
    s->prev = s->next = 0;
}
...
static void lru_push_front(bcache_slot_t *s)
{
    s->prev = 0;
    s->next = g_lru_head;
    if (g_lru_head) g_lru_head->prev = s;
    g_lru_head = s;
...
static int disk_read_block(const blkdev_ops_t *dev, uint32_t lba,
                           uint8_t *buf)
{
...
static int disk_write_block(const blkdev_ops_t *dev, uint32_t lba,
                            const uint8_t *buf)
{
    if (!dev->write_sector) return -1;
    for (uint32_t i = 0; i < BCACHE_SECS_PER_BLK; i++) {
