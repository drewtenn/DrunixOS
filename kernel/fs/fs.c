...
static int inode_read(uint32_t num, dufs_inode_t *out)
{
	...
	if (g_blkdev->read_sector(sector, buf) != 0)
		return -1;
...
{
	uint32_t max_bits =
	    (g_super.total_sectors - g_super.data_lba) / DUFS_SECS_PER_BLOCK;
	if (max_bits > 4096u * 8u)
		max_bits = 4096u * 8u;
...
static uint32_t block_map(const dufs_inode_t *inode, uint32_t block_idx)
{
	...
	if (block_idx < DUFS_INDIR_ENTRIES) {
		if (inode->indirect == 0)
			goto done;
		if (read_block(inode->indirect, buf) != 0)
			goto done;
	...
	if (block_idx < (uint32_t)DUFS_INDIR_ENTRIES * DUFS_INDIR_ENTRIES) {
		if (inode->double_indirect == 0)
			goto done;
		if (read_block(inode->double_indirect, buf) != 0)
			goto done;
...
static uint32_t block_ensure(dufs_inode_t *inode, uint32_t block_idx)
{
	...
	if (block_idx < DUFS_INDIR_ENTRIES) {
		if (inode->indirect == 0) {
			...
			if (indir_lba == 0)
				return 0;
			if (write_zeroed_block(indir_lba) != 0) {
				block_free(indir_lba);
				return 0;
			}
		...
		if (entries[block_idx] == 0) {
			...
			inode->block_count++;
			if (write_block(inode->indirect, buf) != 0) {
				kfree(buf);
				return 0;
			}
		}
	...
	if (block_idx < (uint32_t)DUFS_INDIR_ENTRIES * DUFS_INDIR_ENTRIES) {
		...
		if (inode->double_indirect == 0) {
			...
			if (l0_lba == 0)
				return 0;
			if (write_zeroed_block(l0_lba) != 0) {
				block_free(l0_lba);
				return 0;
			}
		...
		if (l0[l1_idx] == 0) {
			...
			if (write_zeroed_block(l1_lba) != 0) {
				block_free(l1_lba);
				kfree(buf);
				return 0;
			}
			l0[l1_idx] = l1_lba;
			inode->block_count++;
			if (write_block(inode->double_indirect, buf) != 0) {
				kfree(buf);
				return 0;
			}
		}
...
static int path_resolve_follow(const char *path,
                               ...
                               uint32_t depth)
{
	...
	for (;;) {
		...
		if (comp_len == 0 || comp_len >= DUFS_MAX_NAME)
			return -1;
		...
		if (dinode.type == DUFS_TYPE_SYMLINK) {
			...
			if (read_symlink_target(ino, target, 4096) < 0 ||
			    symlink_target_path_with_tail(
			        link_path, target, slash + 1, next, 4096) != 0) {
				kfree(link_path);
				kfree(target);
				kfree(next);
				return -1;
		...
		if (dinode.type != DUFS_TYPE_DIR)
			return -1;

		cur_dir = ino;
		p = slash + 1;
		if (*p == '\0')
			return -1; /* trailing slash — no leaf */
	}
}
...
static int symlink_target_path(const char *link_path,
                               const char *target,
                               char *out,
                               uint32_t outsz)
{
...
int fs_init(void)
{
	...
	uint8_t buf[512];
	if (g_blkdev->read_sector(1, buf) != 0)
		return -1;
	...
	if (read_block(g_super.inode_bitmap_lba, g_inode_bitmap) != 0)
		return -1;
	if (read_block(g_super.block_bitmap_lba, g_block_bitmap) != 0)
		return -1;

	return 0;
}

static int fs_open_follow(const char *path,
                          uint32_t *inode_out,
                          uint32_t *size_out,
                          uint32_t depth)
{
	...
	if (depth > 8)
		return -40;
	if (path_resolve(path, &dir_ino, leaf) != 0)
		return -1;
...
int fs_read(uint32_t inode_num, uint32_t offset, uint8_t *buf, uint32_t count)
{
	...
	while (total_read < count) {
		...
		if (lba == 0)
			break; /* sparse hole or allocation error */

		if (read_block(lba, blk) != 0)
			break;
...
int fs_write(uint32_t inode_num,
             uint32_t offset,
             const uint8_t *buf,
             uint32_t count)
{
...
int fs_unlink(const char *path)
{
	...
	dufs_inode_t inode;
	if (inode_read(child_ino, &inode) != 0)
		return -1;
...
int fs_rmdir(const char *path)
{
	...
	uint32_t par_ino;
	char leaf[DUFS_MAX_NAME];
	if (path_resolve(path, &par_ino, leaf) != 0)
		return -1;
...
int fs_rename(const char *oldpath, const char *newpath)
{
	...
	if (path_resolve(oldpath, &old_dir_ino, old_leaf) != 0)
		return -1;

	uint32_t new_dir_ino;
	char new_leaf[DUFS_MAX_NAME];
	if (path_resolve(newpath, &new_dir_ino, new_leaf) != 0)
		return -1;
	...
	if (inode_read(src_ino, &src_inode) != 0)
		return -1;
	...
	if (dst_ino != 0) {
		if (dst_ino == src_ino)
			return 0; /* same inode — no-op */

		dufs_inode_t dst_inode;
		if (inode_read(dst_ino, &dst_inode) != 0)
			return -1;
		if (dst_inode.type == DUFS_TYPE_DIR)
			return -1; /* cannot replace a dir */

		if (dst_inode.link_count > 0)
			dst_inode.link_count--;
		if (dst_inode.link_count == 0) {
...
static int fs_link_source_inode(const char *path,
                                uint32_t follow,
                                uint32_t depth,
                                uint32_t *ino_out)
{
	...
	if (depth > 8)
		return -40;
	if (path_resolve(path, &old_dir_ino, old_leaf) != 0)
		return -1;
...
int fs_link(const char *oldpath, const char *newpath, uint32_t follow)
{
	...
	if (path_resolve(newpath, &new_dir_ino, new_leaf) != 0)
		return -1;
	if (dir_lookup(new_dir_ino, new_leaf) != 0)
		return -1;
...
int fs_symlink(const char *target, const char *linkpath)
{
	...
	uint32_t dir_ino;
	char leaf[DUFS_MAX_NAME];
	if (path_resolve(linkpath, &dir_ino, leaf) != 0)
		return -1;
	if (dir_lookup(dir_ino, leaf) != 0)
		return -1;
...
int fs_list(const char *path, char *buf, uint32_t bufsz)
{
	...
	if (path == 0 || path[0] == '\0') {
		...
		dufs_inode_t dinode;
		if (inode_read(dir_ino, &dinode) != 0)
			return -1;
		if (dinode.type != DUFS_TYPE_DIR)
			return -1;
	}
...
static int fs_stat_common(const char *path,
                          vfs_stat_t *st,
                          uint32_t follow,
                          uint32_t depth)
{
...
static int fs_stat(const char *path, vfs_stat_t *st)
{
	return fs_stat_common(path, st, 1, 0);
}

static int fs_lstat(const char *path, vfs_stat_t *st)
{
...
static int dufs_vfs_open(void *ctx,
                         const char *path,
                         uint32_t *inode_out,
                         uint32_t *size_out)
{
...
static int dufs_vfs_list(void *ctx, const char *path, char *buf, uint32_t bufsz)
{
	dufs_select(dufs_mount_from_ctx(ctx));
...
static int dufs_vfs_rename(void *ctx, const char *oldpath, const char *newpath)
{
	dufs_select(dufs_mount_from_ctx(ctx));
	return fs_rename(oldpath, newpath);
}

static int dufs_vfs_link(void *ctx,
                         const char *oldpath,
                         const char *newpath,
                         uint32_t follow)
{
...
static int dufs_vfs_symlink(void *ctx, const char *target, const char *linkpath)
{
	dufs_select(dufs_mount_from_ctx(ctx));
...
static int
dufs_vfs_readlink(void *ctx, const char *path, char *buf, uint32_t bufsz)
{
	dufs_select(dufs_mount_from_ctx(ctx));
...
static int dufs_vfs_read(void *ctx,
                         uint32_t inode_num,
                         uint32_t offset,
                         uint8_t *buf,
                         uint32_t count)
...
static int dufs_vfs_write(void *ctx,
                          uint32_t inode_num,
                          uint32_t offset,
                          const uint8_t *buf,
                          uint32_t count)
