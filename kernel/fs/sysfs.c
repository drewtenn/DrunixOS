...
static uint32_t
sysfs_make_inode(uint32_t kind, uint32_t disk_index, uint32_t part_index)
{
...
static int sysfs_append_dirent(
...
static int
sysfs_copy_name(char *dst, uint32_t dstsz, const char *src, uint32_t len)
{
...
static int sysfs_lookup_disk(const char *name,
                             uint32_t *index_out,
...
static int sysfs_lookup_partition(uint32_t disk_index,
                                  const char *name,
                                  uint32_t *index_out,
                                  blkdev_info_t *info_out)
{
	...
	if (idx < 0)
		return -1;
	if (blkdev_info_for_index((uint32_t)idx, &info) != 0 ||
	    info.kind != BLKDEV_KIND_PART || info.parent_index != disk_index)
		return -1;
...
static int sysfs_render_leaf_text(uint32_t inode_num, char *buf, uint32_t bufsz)
{
	...
	case SYSFS_INODE_PART_SIZE:
		if (blkdev_info_for_index(part_index, &part) != 0 ||
		    part.kind != BLKDEV_KIND_PART || part.parent_index != disk_index)
			return -1;
		n = k_snprintf(tmp, sizeof(tmp), "%u\n", part.sectors);
		break;
...
static int sysfs_render_disk_dir(uint32_t disk_index, char *buf, uint32_t bufsz)
{
	...
	for (uint32_t i = 0; i < BLKDEV_MAX; i++) {
		if (blkdev_info_for_index(i, &child) != 0 ||
		    child.kind != BLKDEV_KIND_PART || child.parent_index != disk_index)
			continue;
...
static int sysfs_fill_disk_node(const char *relpath, vfs_node_t *node_out)
{
...

	{
		...
		uint32_t disk_name_len =
		    slash ? (uint32_t)(slash - relpath) : k_strlen(relpath);
		char disk_name[BLKDEV_NAME_MAX];

		if (sysfs_copy_name(
		        disk_name, sizeof(disk_name), relpath, disk_name_len) != 0)
			return -1;
		...
		if (!slash) {
			if (k_strcmp(relpath, "size") == 0) {
				...
				node_out->inode_num =
				    sysfs_make_inode(SYSFS_INODE_DISK_SIZE, disk_index, 0);
				node_out->size =
				    (uint32_t)sysfs_render_leaf_text(node_out->inode_num, 0, 0);
				return 0;
			}
			if (k_strcmp(relpath, "dev") == 0) {
				node_out->type = VFS_NODE_SYSFILE;
				node_out->inode_num =
				    sysfs_make_inode(SYSFS_INODE_DISK_DEV, disk_index, 0);
				node_out->size =
				    (uint32_t)sysfs_render_leaf_text(node_out->inode_num, 0, 0);
				return 0;
			}
			if (k_strcmp(relpath, "type") == 0) {
				node_out->type = VFS_NODE_SYSFILE;
				node_out->inode_num =
				    sysfs_make_inode(SYSFS_INODE_DISK_TYPE, disk_index, 0);
				node_out->size =
				    (uint32_t)sysfs_render_leaf_text(node_out->inode_num, 0, 0);
				return 0;
			}
...

		{
			...
			if (sysfs_copy_name(
			        part_name, sizeof(part_name), relpath, part_name_len) != 0)
				return -1;
			if (sysfs_lookup_partition(
			        disk_index, part_name, &part_index, &part) != 0)
				return -1;

			relpath = slash + 1;
			if (k_strcmp(relpath, "size") == 0) {
				node_out->type = VFS_NODE_SYSFILE;
				node_out->inode_num = sysfs_make_inode(
				    SYSFS_INODE_PART_SIZE, disk_index, part_index);
				node_out->size =
				    (uint32_t)sysfs_render_leaf_text(node_out->inode_num, 0, 0);
				return 0;
			}
			if (k_strcmp(relpath, "dev") == 0) {
				node_out->type = VFS_NODE_SYSFILE;
				node_out->inode_num = sysfs_make_inode(
				    SYSFS_INODE_PART_DEV, disk_index, part_index);
				node_out->size =
				    (uint32_t)sysfs_render_leaf_text(node_out->inode_num, 0, 0);
				return 0;
			}
			if (k_strcmp(relpath, "type") == 0) {
				node_out->type = VFS_NODE_SYSFILE;
				node_out->inode_num = sysfs_make_inode(
				    SYSFS_INODE_PART_TYPE, disk_index, part_index);
				node_out->size =
				    (uint32_t)sysfs_render_leaf_text(node_out->inode_num, 0, 0);
				return 0;
			}
			if (k_strcmp(relpath, "partition") == 0) {
				node_out->type = VFS_NODE_SYSFILE;
				node_out->inode_num = sysfs_make_inode(
				    SYSFS_INODE_PARTITION, disk_index, part_index);
				node_out->size =
				    (uint32_t)sysfs_render_leaf_text(node_out->inode_num, 0, 0);
				return 0;
			}
			if (k_strcmp(relpath, "start") == 0) {
				node_out->type = VFS_NODE_SYSFILE;
				node_out->inode_num =
				    sysfs_make_inode(SYSFS_INODE_START, disk_index, part_index);
				node_out->size =
				    (uint32_t)sysfs_render_leaf_text(node_out->inode_num, 0, 0);
				return 0;
			}
...
int sysfs_getdents(const char *relpath, char *buf, uint32_t bufsz)
{
...

	{
		...
		uint32_t disk_name_len =
		    slash ? (uint32_t)(slash - tail) : k_strlen(tail);
		char disk_name[BLKDEV_NAME_MAX];
		uint32_t disk_index = 0;
		blkdev_info_t disk;

		if (sysfs_copy_name(
		        disk_name, sizeof(disk_name), tail, disk_name_len) != 0)
			return -1;
		if (sysfs_lookup_disk(disk_name, &disk_index, &disk) != 0)
			return -1;
...

		{
			...
			if (sysfs_copy_name(
			        part_name, sizeof(part_name), tail, part_name_len) != 0)
				return -1;
