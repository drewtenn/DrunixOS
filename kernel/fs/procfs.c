...
static int procfs_append_dirent(char *buf, uint32_t bufsz, uint32_t *written,
...
static int procfs_parse_u32(const char *s, uint32_t *value_out, const char **end_out)
{
    uint32_t value = 0;
    int digits = 0;
...
static int procfs_parse_path(const char *relpath, uint32_t *node_type_out,
                             uint32_t *kind_out, uint32_t *pid_out,
                             uint32_t *index_out)
{
    uint32_t pid = 0;
    uint32_t index = 0;
    ...
    if (k_strcmp(relpath, "kmsg") == 0) {
        ...
        if (kind_out) *kind_out = PROCFS_FILE_KMSG;
        if (pid_out) *pid_out = 0;
        if (index_out) *index_out = 0;
        return 0;
    }
    ...
    if (k_strcmp(relpath, "modules") == 0) {
        if (node_type_out) *node_type_out = VFS_NODE_PROCFILE;
        if (kind_out) *kind_out = PROCFS_FILE_MODULES;
        if (pid_out) *pid_out = 0;
        if (index_out) *index_out = 0;
        return 0;
    }
    ...
    if (k_strcmp(slash, "status") == 0) {
        if (node_type_out) *node_type_out = VFS_NODE_PROCFILE;
        if (kind_out) *kind_out = PROCFS_FILE_STATUS;
        if (pid_out) *pid_out = pid;
        if (index_out) *index_out = 0;
        return 0;
    }
    ...
    if (k_strcmp(slash, "maps") == 0) {
        if (node_type_out) *node_type_out = VFS_NODE_PROCFILE;
        if (kind_out) *kind_out = PROCFS_FILE_MAPS;
        if (pid_out) *pid_out = pid;
        if (index_out) *index_out = 0;
        return 0;
    }
    ...
    if (k_strncmp(slash, "fd/", 3) == 0) {
        ...
        if (index >= MAX_FDS || proc->open_files[index].type == FD_TYPE_NONE)
            return -1;
        if (node_type_out) *node_type_out = VFS_NODE_PROCFILE;
        if (kind_out) *kind_out = PROCFS_FILE_FD;
        if (pid_out) *pid_out = pid;
...
static void procfs_render_fd(render_buf_t *rb, const process_t *proc, uint32_t fd)
{
    ...
    case FD_TYPE_PIPE_WRITE:
        ...
        break;
    case FD_TYPE_TTY:
        procfs_emitf(rb, "type=tty index=%u writable=%u\n",
                     fh->u.tty.tty_idx, fh->writable);
        break;
    case FD_TYPE_PROCFILE:
        procfs_emitf(rb, "type=procfs kind=%s pid=%u index=%u\n",
...
static void procfs_render_modules(render_buf_t *rb)
{
    ...
    for (int i = 0; i < n; i++) {
        procfs_emitf(rb, "%s %u 0x%08x live\n",
                     mods[i].name, mods[i].size, mods[i].base);
    }
}
                     ...
                     uint32_t offset, char *buf, uint32_t count)
{
    ...
    if (procfs_render_file(kind, pid, index, render, PROCFS_RENDER_CAP, &size) != 0)
    {
        kfree(render);
        return -1;
    }
