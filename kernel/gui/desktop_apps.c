...
static void
desktop_app_set_line(desktop_app_view_t *view, int line, const char *text)
{
	uint32_t len = 0;
...
static void desktop_app_mark_truncated(desktop_app_view_t *view)
{
	...
	if (view->line_count < DESKTOP_APP_MAX_LINES)
		...
		desktop_app_set_line(
		    view, DESKTOP_APP_MAX_LINES - 1, "... more entries");
}
                        ...
                        const gui_pixel_rect_t *rect)
{
	...
	framebuffer_fill_rect(
	    surface->fb, clip.x, clip.y, clip.w, clip.h, theme->window_bg);
	...
	for (line = start_line; line < view->line_count; line++) {
		...
		if (!desktop_app_safe_int64_to_int(line_y, &safe_y))
			return;
		framebuffer_draw_text_clipped(surface->fb,
		                              &clip,
		                              safe_x,
		                              safe_y,
		                              view->lines[line],
