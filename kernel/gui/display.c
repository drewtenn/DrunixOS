...
static void
gui_display_vga_color(uint8_t color, uint8_t *r, uint8_t *g, uint8_t *b)
{
...
static gui_rect_t
gui_clip_rect(const gui_display_t *display, int x, int y, int w, int h)
{
...
void gui_display_init(gui_display_t *display,
                      gui_cell_t *cells,
                      int cols,
                      int rows,
...
gui_rect_t gui_display_fill_rect(
    gui_display_t *display, int x, int y, int w, int h, char ch, uint8_t attr)
{
...
gui_rect_t gui_display_draw_frame(
    gui_display_t *display, int x, int y, int w, int h, uint8_t attr)
{
...
void gui_display_present_to_vga(const gui_display_t *display,
...
{
	gui_display_present_rect_to_framebuffer(display,
	                                        fb,
	                                        0,
	                                        0,
	...
	for (int row = dirty.y; row < dirty.y + dirty.h; row++) {
		for (int col = dirty.x; col < dirty.x + dirty.w; col++) {
			...
			framebuffer_draw_glyph(
			    fb, col * 8, row * 16, (unsigned char)cell->ch, fg, bg);
		}
	}
}
