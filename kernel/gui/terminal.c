...
static void
terminal_vga_color(uint8_t color, uint8_t *r, uint8_t *g, uint8_t *b);
                                        ...
                                        uint32_t *row_bytes)
{
	...
	if (history_rows > 0) {
		history_cells =
		    (uint64_t)(uint32_t)cols * (uint64_t)(uint32_t)history_rows;
...
static int terminal_cell_count_u32(const gui_terminal_t *term, uint32_t *cells)
{
	...
	total_cells =
	    (uint64_t)(uint32_t)term->cols * (uint64_t)(uint32_t)term->rows;
...
static void terminal_push_history(gui_terminal_t *term, const gui_cell_t *row)
{
	...
	if (term->history_count < capacity) {
		tail = ((uint32_t)term->history_head + (uint32_t)term->history_count) %
		       capacity;
		term->history_count++;
	} else {
...
static int
terminal_csi_param(const gui_terminal_t *term, int index, int fallback)
{
                            ...
                            uint8_t default_attr)
{
	...
	if (!terminal_validate_dimensions(
	        cols, rows, history_rows, &live_bytes, &history_bytes, 0))
		return 0;
	...
	if (!gui_terminal_init_static(
	        term, live, history, cols, rows, history_rows, default_attr)) {
		if (history)
			kfree(history);
		kfree(live);
		return 0;
                                 ...
                                 const gui_cell_t *cell)
{
	...
	if (bg != theme->terminal_bg) {
		...
		for (int row = 0; row < bg_h; row++) {
			uint32_t *pixels =
			    (uint32_t *)(base + (uintptr_t)(bg_y + row) * row_pitch);
                         ...
                         int draw_cursor)
{
	...
	if (!term || !surface || !surface->fb || !theme)
		return;
	if (!terminal_intersect_pixel_rect(term->pixel_rect, surface->clip, &clip))
		return;

	framebuffer_fill_rect(
	    surface->fb, clip.x, clip.y, clip.w, clip.h, theme->terminal_bg);
	...
	for (int row = 0; row < term->rows; row++) {
		...
		for (int col = 0; col < term->cols; col++) {
			...
			terminal_render_cell(
			    surface->fb, term, theme, &clip, cell_x, cell_y, &cells[col]);
		}
	}

	if (draw_cursor && term->cursor_x >= 0 && term->cursor_x < term->cols &&
	    term->cursor_y >= 0) {
		int64_t visible_cursor_row =
		    (int64_t)term->cursor_y + (int64_t)term->view_top;
		int64_t cursor_x =
		    content_x + (int64_t)term->cursor_x * (int64_t)GUI_FONT_W;
		...
		if (visible_cursor_row >= 0 &&
		    visible_cursor_row < (int64_t)term->rows && cursor_x >= INT_MIN &&
		    cursor_x <= (int64_t)INT_MAX - (int64_t)GUI_FONT_W + 1 &&
		    cursor_y >= INT_MIN && cursor_y <= (int64_t)INT_MAX - 2) {
			cursor_rect.x = (int)cursor_x;
			cursor_rect.y = (int)cursor_y;
			cursor_rect.w = (int)GUI_FONT_W;
			cursor_rect.h = 2;
			if (terminal_intersect_pixel_rect(
			        cursor_rect, clip, &cursor_clip)) {
