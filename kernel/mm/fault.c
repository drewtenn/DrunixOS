...
static int paging_handle_lazy_anon_fault(uint32_t pd_phys, uint32_t cr2,
                                         uint32_t user_esp, process_t *cur,
                                         const vm_area_t *vma)
{
    ...
    if (vma->kind == VMA_KIND_STACK &&
        (vma->flags & VMA_FLAG_GROWSDOWN) != 0 &&
        fault_page >= vma->start &&
        fault_page < cur->stack_low_limit &&
        cr2 >= stack_slack &&
        cr2 < vma->end) {
