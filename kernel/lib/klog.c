...
static int klog_format_line(char *buf, uint32_t cap,
                            uint32_t ticks, klog_level_t level,
                            const char *tag, const char *msg)
{
    uint32_t secs = ticks / SCHED_HZ;
    uint32_t ms = ((ticks % SCHED_HZ) * 1000u) / SCHED_HZ;

    return k_snprintf(buf, cap, "[%u.%03u] %s [%s] %s\n",
                      secs, ms, klog_level_name(level), tag, msg);
}
