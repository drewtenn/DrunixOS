...
{
    while (count-- > 0) ob_putc(ob, padch);
}
 ...
 */
static void fmt_uint(outbuf_t *ob, uint32_t val, int base, int upper,
                     int flags, int width, char sign)
{
    ...
    if (flags & F_LEFT) {
        ...
        if (sign) ob_putc(ob, sign);
        for (int i = len - 1; i >= 0; i--) ob_putc(ob, tmp[i]);
        ob_pad(ob, ' ', pad);
...
int k_vsnprintf(char *buf, uint32_t size, const char *fmt, va_list ap)
{
    ...
    while (*fmt) {
        ...
        int flags = 0;
        for (;;) {
            if      (*fmt == '-') { flags |= F_LEFT;  fmt++; }
            else if (*fmt == '0') { flags |= F_ZERO;  fmt++; }
            else if (*fmt == '+') { flags |= F_PLUS;  fmt++; }
            else if (*fmt == ' ') { flags |= F_SPACE; fmt++; }
            else break;
        }
        ...
        case 'd': case 'i':
            ...
            break;
        case 'u':
            fmt_uint(&ob, (uint32_t)va_arg(ap, unsigned int),
                     10, 0, flags, width, '\0');
            break;
        case 'x':
            fmt_uint(&ob, (uint32_t)va_arg(ap, unsigned int),
        ...
        case 'p':
            ob_putc(&ob, '0');
            ob_putc(&ob, 'x');
            fmt_uint(&ob, (uint32_t)va_arg(ap, void *),
                     16, 0, F_ZERO, 8, '\0');
            break;
        case 's':
