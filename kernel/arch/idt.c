...
static const char *exception_name(uint32_t vec)
{
    static const char *names[] = {
        ...
        "#MC Machine Check",         "#XM SIMD FPU Exception",
        "#VE Virtualisation",        "#21 Reserved",
    };
    if (vec < 22) return names[vec];
...
static void panic_hex(const char *label, uint32_t val)
{
    ...
    buf[i++] = '['; buf[i++] = 'P'; buf[i++] = 'A'; buf[i++] = 'N';
    buf[i++] = 'I'; buf[i++] = 'C'; buf[i++] = ']'; buf[i++] = ' ';
    for (int j = 0; label[j]; j++) buf[i++] = label[j];
    buf[i++] = '=';
    buf[i++] = '0';
    buf[i++] = 'x';
...
{
    switch (vec) {
        case 0:  return SIGFPE;   /* #DE */
        case 1:  return SIGTRAP;  /* #DB */
        case 3:  return SIGTRAP;  /* #BP */
        case 6:  return SIGILL;   /* #UD */
        case 13: return SIGSEGV;  /* #GP */
        case 14: return SIGSEGV;  /* #PF */
        case 16: return SIGFPE;   /* x87 floating point */
        case 17: return SIGSEGV;  /* alignment check */
        case 19: return SIGFPE;   /* SIMD floating point */
        default: return 0;
    }
}
...
void isr_handler(trap_frame_t *f)
{
    if ((f->cs & 3) == 3) {
        ...
        if (f->vector == 14) {
            ...
            if (paging_handle_fault(cur ? cur->pd_phys : 0,
                                    cr2, f->error_code, f->user_esp, cur) == 0)
                return;
        }
    ...
    if (f->vector == 13 && f->error_code != 0) {
        ...
        /* bit 1 = IDT, bit 0 = EXT */
        if (e & 2) panic_puts("[PANIC] source=IDT\n");
        else if (e & 4) panic_puts("[PANIC] source=LDT\n");
        else            panic_puts("[PANIC] source=GDT\n");
    }
    ...
    if (f->vector == 14) {
        ...
        if (!(f->error_code & 1)) panic_puts("not-present ");
        if   (f->error_code & 2)  panic_puts("write ");
        if   (f->error_code & 4)  panic_puts("user ");
        panic_puts("\n");
    }

    /* General-purpose registers */
    panic_hex("EAX", f->eax); panic_hex("EBX", f->ebx);
    panic_hex("ECX", f->ecx); panic_hex("EDX", f->edx);
    panic_hex("ESI", f->esi); panic_hex("EDI", f->edi);
    panic_hex("EBP", f->ebp);
