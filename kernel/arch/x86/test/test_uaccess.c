...
#define TEST_IMAGE_PAGE 0x10008000u
#define TEST_BSS_ADDR 0x100086A0u
...
static void test_copy_from_user_reads_mapped_bytes(ktest_case_t *tc)
{
	...
	if (!map_user_page(&proc, 0x10040000u, 0x00)) {
		KTEST_EXPECT_TRUE(tc, 0);
		destroy_test_proc(&proc);
		return;
	}

	src = mapped_alias(&proc, 0x10040080u);
	if (!src) {
	...
	KTEST_EXPECT_EQ(
	    tc, uaccess_copy_from_user(&proc, buf, 0x10040080u, 4u), 0u);
...
static void test_copy_to_user_spans_pages(ktest_case_t *tc)
{
	...
	uint8_t *dst0;
	uint8_t *dst1;
	uint32_t start = 0x10041000u + PAGE_SIZE - 50u;
	...
	if (!map_user_page(&proc, 0x10041000u, 0xAA) ||
	    !map_user_page(&proc, 0x10042000u, 0xBB)) {
		KTEST_EXPECT_TRUE(tc, 0);
		destroy_test_proc(&proc);
		return;
	}
	...
	dst1 = mapped_alias(&proc, 0x10042000u);
	if (!dst0 || !dst1) {
...
static void test_copy_string_from_user_spans_pages(ktest_case_t *tc)
{
	...
	uint32_t start = 0x10041000u + PAGE_SIZE - 6u;
	...
	if (!map_user_page(&proc, 0x10041000u, 0x00) ||
	    !map_user_page(&proc, 0x10042000u, 0x00)) {
		KTEST_EXPECT_TRUE(tc, 0);
		destroy_test_proc(&proc);
		return;
	}
...
static void test_prepare_rejects_kernel_and_unmapped_ranges(ktest_case_t *tc)
{
	...
	if (!map_user_page(&proc, 0x10043000u, 0x00)) {
		KTEST_EXPECT_TRUE(tc, 0);
		destroy_test_proc(&proc);
		return;
	}
	...
	KTEST_EXPECT_EQ(tc,
	                uaccess_prepare(&proc, 0x10043000u + PAGE_SIZE - 2u, 4u, 0),
	                (uint32_t)-1);
	KTEST_EXPECT_EQ(
...
static void test_copy_to_user_breaks_cow_clone(ktest_case_t *tc)
{
	...
	if (!map_user_page(&parent, 0x10044000u, 0x00)) {
		KTEST_EXPECT_TRUE(tc, 0);
	...
	if (uaccess_copy_to_user(
	        &parent, 0x10044000u, parent_msg, sizeof(parent_msg)) != 0) {
		KTEST_EXPECT_TRUE(tc, 0);
		destroy_test_proc(&parent);
		return;
	}
	...
	if (paging_walk(parent.pd_phys, 0x10044000u, &parent_pte) != 0 ||
	    paging_walk(child.pd_phys, 0x10044000u, &child_pte) != 0) {
		KTEST_EXPECT_TRUE(tc, 0);
	...
	KTEST_EXPECT_EQ(
	    tc,
	    uaccess_copy_to_user(&child, 0x10044000u, child_msg, sizeof(child_msg)),
	    0u);
	...
	KTEST_EXPECT_EQ(
	    tc,
	    k_strcmp((char *)mapped_alias(&parent, 0x10044000u), parent_msg),
	    0u);
	KTEST_EXPECT_EQ(
	    tc, k_strcmp((char *)mapped_alias(&child, 0x10044000u), child_msg), 0u);
...
test_release_user_space_drops_only_child_cow_reference(ktest_case_t *tc)
{
	...
	if (!map_user_page(&parent, 0x10045000u, 0x00)) {
		KTEST_EXPECT_TRUE(tc, 0);
		destroy_test_proc(&parent);
		return;
	}

	if (uaccess_copy_to_user(&parent, 0x10045000u, msg, sizeof(msg)) != 0) {
		KTEST_EXPECT_TRUE(tc, 0);
		destroy_test_proc(&parent);
		return;
	}
	...
	if (paging_walk(parent.pd_phys, 0x10045000u, &parent_pte) != 0 ||
	    paging_walk(child.pd_phys, 0x10045000u, &child_pte) != 0) {
		KTEST_EXPECT_TRUE(tc, 0);
		destroy_test_proc(&child);
		destroy_test_proc(&parent);
		return;
	...
	KTEST_EXPECT_EQ(
	    tc, k_strcmp((char *)mapped_alias(&parent, 0x10045000u), msg), 0u);
...
static void test_copy_to_user_handles_three_way_cow_sharing(ktest_case_t *tc)
{
	...
	if (!map_user_page(&parent, 0x10046000u, 0x00)) {
		KTEST_EXPECT_TRUE(tc, 0);
		destroy_test_proc(&parent);
		return;
	}
	...
	if (uaccess_copy_to_user(
	        &parent, 0x10046000u, parent_msg, sizeof(parent_msg)) != 0) {
		KTEST_EXPECT_TRUE(tc, 0);
		destroy_test_proc(&parent);
		return;
	}
	...
	if (paging_walk(parent.pd_phys, 0x10046000u, &parent_pte) != 0 ||
	    paging_walk(child1.pd_phys, 0x10046000u, &child1_pte) != 0 ||
	    paging_walk(child2.pd_phys, 0x10046000u, &child2_pte) != 0) {
		KTEST_EXPECT_TRUE(tc, 0);
		destroy_test_proc(&child2);
		destroy_test_proc(&child1);
		destroy_test_proc(&parent);
	...
	KTEST_EXPECT_EQ(tc,
	                uaccess_copy_to_user(
	                    &child2, 0x10046000u, child2_msg, sizeof(child2_msg)),
	                0u);
	...
	KTEST_EXPECT_EQ(tc,
	                uaccess_copy_to_user(
	                    &child1, 0x10046000u, child1_msg, sizeof(child1_msg)),
	                0u);
	...
	KTEST_EXPECT_EQ(
	    tc,
	    k_strcmp((char *)mapped_alias(&parent, 0x10046000u), parent_msg),
	    0u);
	KTEST_EXPECT_EQ(
	    tc,
	    k_strcmp((char *)mapped_alias(&child1, 0x10046000u), child1_msg),
	    0u);
	KTEST_EXPECT_EQ(
	    tc,
	    k_strcmp((char *)mapped_alias(&child2, 0x10046000u), child2_msg),
	    0u);
