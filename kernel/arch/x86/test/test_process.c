...
static void init_core_dump_proc(process_t *proc)
{
	...
	proc->crash.frame.eip = 0x08002A16u;
}
...
static void test_process_build_initial_frame_layout(ktest_case_t *tc)
{
	...
	init_frame_proc(&proc, kstack_words, 0x08001000u, 0xBFFFE000u);
	...
	KTEST_EXPECT_EQ(tc, frame[19], 0x08001000u);
...
static void test_process_build_exec_frame_layout(ktest_case_t *tc)
{
	...
	init_frame_proc(&proc, kstack_words, 0x08002000u, 0xBFFFD000u);
	...
	KTEST_EXPECT_EQ(tc, frame[7], GDT_USER_DS);
	KTEST_EXPECT_EQ(tc, frame[21], 0x08002000u);
...
static void test_elf_loader_rejects_overlapping_pt_loads(ktest_case_t *tc)
{
	...
	phdrs[0] = test_elf_load_phdr(0x08000000u, 0x3000u);
	phdrs[1] = test_elf_load_phdr(0x08002000u, PAGE_SIZE);
...
test_sched_add_builds_initial_frame_for_never_run_process(ktest_case_t *tc)
{
	...
	sched_init();
	init_frame_proc(&proc, kstack_words, 0x08003000u, 0xBFFFC000u);
	...
	KTEST_ASSERT_NOT_NULL(tc, frame);
	KTEST_EXPECT_EQ(tc, frame[4], (uint32_t)process_initial_launch);
	KTEST_EXPECT_EQ(tc, frame[19], 0x08003000u);
...
static void test_vma_add_keeps_regions_sorted_and_findable(ktest_case_t *tc)
{
	...
	KTEST_EXPECT_NULL(tc, vma_find(&proc, 0x08100000u));
}
...
static void test_syscall_fstat_reads_resource_fd_table(ktest_case_t *tc)
{
	...
	KTEST_EXPECT_EQ(tc,
	                syscall_handler(SYS_FSTAT64, 1, 0x08100400u, 0, 0, 0, 0),
	                (uint32_t)-1);
...
static void test_syscall_getcwd_reads_resource_fs_state(ktest_case_t *tc)
{
	...
	uint8_t *page;
	KTEST_ASSERT_NOT_NULL(tc, cur);
	KTEST_ASSERT_EQ(tc, map_test_user_page(cur, 0x08100000u), 0u);
	page = mapped_alias(cur, 0x08100000u);
	KTEST_ASSERT_NOT_NULL(tc, page);

	k_strncpy(cur->fs_state->cwd, "home", sizeof(cur->fs_state->cwd) - 1u);

	KTEST_EXPECT_EQ(
	    tc, syscall_handler(SYS_GETCWD, 0x08100000u, 64, 0, 0, 0, 0), 6u);
...
static void test_syscall_chdir_updates_resource_fs_state(ktest_case_t *tc)
{
	...
	KTEST_ASSERT_EQ(
	    tc, syscall_handler(SYS_CHDIR, 0x08100000u, 0, 0, 0, 0, 0), 0u);
	KTEST_EXPECT_TRUE(tc, k_strcmp(cur->fs_state->cwd, "bin") == 0);
	KTEST_EXPECT_EQ(
	    tc, syscall_handler(SYS_GETCWD, 0x08100100u, 64, 0, 0, 0, 0), 5u);
...
static void test_rt_sigaction_reads_resource_handlers(ktest_case_t *tc)
{
	...
	KTEST_EXPECT_EQ(
	    tc,
	    syscall_handler(SYS_RT_SIGACTION, SIGTERM, 0, 0x08100000u, 8, 0, 0),
	    0u);
...
static void test_linux_syscalls_fill_uname_time_and_fstat64(ktest_case_t *tc)
{
	...
	KTEST_EXPECT_EQ(
	    tc, syscall_handler(SYS_UNAME, 0x08100000u, 0, 0, 0, 0, 0), 0u);
	...
	KTEST_EXPECT_EQ(
	    tc,
	    syscall_handler(SYS_GETTIMEOFDAY, 0x08100200u, 0x08100210u, 0, 0, 0, 0),
	    0u);
	KTEST_EXPECT_EQ(tc, page[0x210], 0u);
	KTEST_EXPECT_EQ(tc, page[0x211], 0u);

	KTEST_EXPECT_EQ(
	    tc, syscall_handler(SYS_FSTAT64, 1, 0x08100400u, 0, 0, 0, 0), 0u);
...
static void test_linux_syscalls_cover_blockdev_fd_path(ktest_case_t *tc)
{
	...
	KTEST_EXPECT_EQ(
	    tc,
	    syscall_handler(SYS_STAT64, 0x08100000u, 0x08100100u, 0, 0, 0, 0),
	    0u);
	...
	fd = (int32_t)syscall_handler(SYS_OPEN, 0x08100000u, 0u, 0, 0, 0, 0);
	...
	KTEST_EXPECT_EQ(
	    tc,
	    syscall_handler(SYS_FSTAT64, (uint32_t)fd, 0x08100180u, 0, 0, 0, 0),
	    0u);
	...
	fd_part = (int32_t)syscall_handler(SYS_OPEN, 0x08100000u, 0u, 0, 0, 0, 0);
	...
	KTEST_EXPECT_EQ(
	    tc,
	    syscall_handler(
	        SYS_FSTAT64, (uint32_t)fd_part, 0x08100180u, 0, 0, 0, 0),
	    0u);
	...
	KTEST_EXPECT_EQ(
	    tc, syscall_handler(SYS_POLL, 0x08100200u, 1u, 0, 0, 0, 0), 1u);
	KTEST_EXPECT_EQ(
	...
	KTEST_EXPECT_EQ(
	    tc,
	    syscall_handler(SYS_READ, (uint32_t)fd, 0x08100300u, 513u, 0, 0, 0),
	    513u);
	...
	KTEST_EXPECT_EQ(
	    tc,
	    syscall_handler(SYS_READ, (uint32_t)fd, 0x08100300u, 1u, 0, 0, 0),
	    1u);
	...
	KTEST_EXPECT_EQ(
	    tc,
	    syscall_handler(SYS_WRITE, (uint32_t)fd, 0x08100100u, 1u, 0, 0, 0),
	    (uint32_t)-1);
	KTEST_EXPECT_EQ(
	...
	KTEST_EXPECT_EQ(
	    tc,
	    syscall_handler(SYS_LSTAT64, 0x08100000u, 0x08100100u, 0, 0, 0, 0),
	    0u);
	stat_mode = test_read_u32_le(stat_base, 16u);
	KTEST_EXPECT_EQ(tc, stat_mode, 0x0000A1FFu);
	k_strcpy((char *)page, "/sys/block/sda/size");
	sys_fd = (int32_t)syscall_handler(SYS_OPEN, 0x08100000u, 0u, 0, 0, 0, 0);
	...
	KTEST_EXPECT_EQ(
	    tc,
	    syscall_handler(SYS_READ, (uint32_t)sys_fd, 0x08100300u, 5u, 0, 0, 0),
	    5u);
	read_base[5] = '\0';
	...
	KTEST_EXPECT_EQ(
	    tc,
	    syscall_handler(SYS_WRITE, (uint32_t)sys_fd, 0x08100100u, 1u, 0, 0, 0),
	    (uint32_t)-TEST_LINUX_EBADF);
	KTEST_EXPECT_EQ(
	    tc,
	    syscall_handler(SYS_FSTAT64, (uint32_t)sys_fd, 0x08100180u, 0, 0, 0, 0),
	    0u);
	fstat_mode = test_read_u32_le(fstat_base, 16u);
...
test_linux_syscalls_support_busybox_identity_and_rt_sigmask(ktest_case_t *tc)
{
	...
	KTEST_EXPECT_EQ(
	    tc,
	    syscall_handler(
	        SYS_RT_SIGPROCMASK, 0, 0x08100100u, 0x08100200u, 8, 0, 0),
	    0u);
	...
	KTEST_EXPECT_EQ(
	    tc,
	    syscall_handler(
	        SYS_RT_SIGPROCMASK, 1, 0x08100100u, 0x08100200u, 8, 0, 0),
	    0u);
...
static void test_linux_poll_and_select_wait_for_tty_input(ktest_case_t *tc)
{
	...
	((uint32_t *)poll_base)[1] = TEST_LINUX_POLLIN;
	KTEST_EXPECT_EQ(
	    tc, syscall_handler(SYS_POLL, 0x08100100u, 1u, 0, 0, 0, 0), 0u);
	KTEST_EXPECT_EQ(tc, ((uint32_t *)poll_base)[1] >> 16, 0u);

	readfds = (uint32_t *)(page + 0x200u);
	*readfds = 1u;
	KTEST_EXPECT_EQ(
	    tc, syscall_handler(SYS__NEWSELECT, 1u, 0x08100200u, 0, 0, 0, 0), 0u);
	KTEST_EXPECT_EQ(tc, *readfds, 0u);

	tty_input_char(0, 'q');
	((uint32_t *)poll_base)[1] = TEST_LINUX_POLLIN;
	KTEST_EXPECT_EQ(
	    tc, syscall_handler(SYS_POLL, 0x08100100u, 1u, 0, 0, 0, 0), 1u);
...
test_linux_termios_on_stdout_controls_foreground_tty(ktest_case_t *tc)
{
	...
	KTEST_EXPECT_EQ(
	    tc,
	    syscall_handler(SYS_IOCTL, 1u, TEST_LINUX_TCGETS, 0x08100100u, 0, 0, 0),
	    0u);
	...
	KTEST_EXPECT_EQ(
	    tc,
	    syscall_handler(SYS_IOCTL, 1u, TEST_LINUX_TCSETS, 0x08100100u, 0, 0, 0),
	    0u);
...
static void test_linux_syscalls_support_busybox_stdio_helpers(ktest_case_t *tc)
{
	...
	page[0x111] = (uint8_t)'\n';
	KTEST_EXPECT_EQ(
	    tc, syscall_handler(SYS_WRITEV, 1, 0x08100000u, 2, 0, 0, 0), 5u);
	KTEST_EXPECT_EQ(
	    tc, syscall_handler(SYS_WRITE, 1, 0x08100100u, 3, 0, 0, 0), 3u);

	KTEST_EXPECT_EQ(
	    tc, syscall_handler(SYS_GETCWD, 0x08100500u, 32, 0, 0, 0, 0), 2u);
	...
	KTEST_EXPECT_EQ(
	    tc,
	    syscall_handler(
	        SYS_IOCTL, 1, TEST_LINUX_TIOCGWINSZ, 0x08100200u, 0, 0, 0),
	    0u);
	...
	KTEST_EXPECT_EQ(
	    tc, syscall_handler(SYS__LLSEEK, 3, 0, 5, 0x08100300u, 1, 0), 0u);
	...
	u32[0x404 / 4] = 0u;
	KTEST_EXPECT_EQ(
	    tc, syscall_handler(SYS_SENDFILE64, 1, 3, 0x08100400u, 0, 0, 0), 0u);
...
test_linux_open_create_append_preserves_flags_and_data(ktest_case_t *tc)
{
	...
	fd = syscall_handler(
	    SYS_OPEN, 0x08100000u, TEST_LINUX_O_CREAT, 0644u, 0, 0, 0);
	...
	KTEST_EXPECT_EQ(tc,
	                syscall_handler(SYS_WRITE, fd, 0x08100100u, 1u, 0, 0, 0),
	                (uint32_t)-TEST_LINUX_EBADF);
	...
	KTEST_EXPECT_EQ(
	    tc, syscall_handler(SYS_WRITE, fd, 0x08100200u, 1u, 0, 0, 0), 1u);
	KTEST_EXPECT_EQ(tc, syscall_handler(SYS_CLOSE, fd, 0, 0, 0, 0, 0), 0u);
...
static void test_linux_syscalls_install_tls_and_map_mmap2(ktest_case_t *tc)
{
	...
	KTEST_ASSERT_NOT_NULL(tc, cur);
	KTEST_ASSERT_EQ(tc, map_test_user_page(cur, 0x08101000u), 0u);
	page = mapped_alias(cur, 0x08101000u);
	KTEST_ASSERT_NOT_NULL(tc, page);
	...
	KTEST_EXPECT_EQ(
	    tc,
	    syscall_handler(SYS_SET_THREAD_AREA, 0x08101000u, 0, 0, 0, 0, 0),
	    0u);
	KTEST_EXPECT_EQ(tc, desc[0], GDT_USER_TLS_ENTRY);
	KTEST_EXPECT_EQ(
	    tc,
	    syscall_handler(SYS_SET_TID_ADDRESS, 0x08101080u, 0, 0, 0, 0, 0),
	    cur->pid);
