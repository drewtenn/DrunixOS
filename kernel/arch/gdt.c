...
static void gdt_set_entry(
    int idx, uint32_t base, uint32_t limit, uint8_t access, uint8_t flags)
{
...
void gdt_init(void)
{
	...
	tss.esp0 =
	    0x90000; /* placeholder; replaced with per-process kstack_top before any ring-3 → ring-0 transition */
	tss.iomap_base =
	    (uint16_t)sizeof(tss_t); /* beyond TSS end = deny all port I/O */
...
#ifdef KTEST_ENABLED
void gdt_get_user_tls_for_test(uint32_t *base_out,
                               uint32_t *limit_out,
                               int *limit_in_pages_out,
                               int *present_out)
{
