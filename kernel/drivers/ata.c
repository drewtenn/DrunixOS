...
static void ata_setup_lba(uint32_t lba, uint8_t slave)
{
	...
	port_byte_out(ATA_PRIMARY_BASE + ATA_REG_DRV_HEAD,
	              0xE0 | (slave ? 0x10 : 0x00) | ((uint8_t)(lba >> 24) & 0x0F));
	ata_delay400();
...
static int
ata_write_sector_drive(uint32_t lba, const uint8_t *buf, uint8_t slave)
{
	if (ata_wait_bsy() != ATA_OK)
		return ATA_TIMEOUT;
	ata_setup_lba(lba, slave);
...
{
	blkdev_register_disk("sda", 8u, 0u, DRUNIX_DISK_SECTORS, &ata_master_ops);
	blkdev_register_disk("sdb", 8u, 16u, DRUNIX_DISK_SECTORS, &ata_slave_ops);
}
