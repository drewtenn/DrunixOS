...
int blkdev_register_disk(const char *name,
                         uint32_t major,
                         uint32_t minor,
                         uint32_t sectors,
                         const blkdev_ops_t *ops)
{
	...
	if (!blkdev_name_is_valid(name) || !ops || sectors == 0 ||
...
int blkdev_register_part(const char *name,
                         ...
                         uint32_t sectors)
{
	...
	if (!blkdev_name_is_valid(name) || sectors == 0 ||
	    parent_index >= BLKDEV_MAX || blkdev_find_index(name) >= 0)
		return -1;
