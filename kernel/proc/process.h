...
typedef struct process {
	...
	wait_queue_t
	    *wait_queue;      /* queue this process is blocked on, or NULL       */
	process_t *wait_next; /* intrusive link while queued                      */
	uint32_t
	    wait_deadline; /* scheduler tick deadline for timed waits         */
	uint32_t wait_deadline_set; /* 1 when wait_deadline is active             */
	uint32_t
	    heap_start; /* first byte of the heap region (page-rounded BSS end) */
