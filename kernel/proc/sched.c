...
extern void switch_context(uint32_t *old_esp_ptr, uint32_t new_esp, uint32_t new_cr3);
...
static process_t *sched_pick_next(void)
{
    ...
    if (g_current) {
        for (int i = 0; i < MAX_PROCS; i++) {
            if (&proc_table[i] == g_current) { cur_idx = i; break; }
        }
    }
...
void sched_set_exit_status(uint32_t status)
{
    ...
    if (g_current) g_current->exit_status = (status & 0xFF) << 8;
}
...
void schedule(void)
{
    ...
    uint32_t *save_ptr = (prev && prev->state != PROC_ZOMBIE)
                         ? &prev->saved_esp : (uint32_t *)0;
...
{
    if (signum < 1 || signum >= NSIG || pgid == 0) return;
    for (int i = 0; i < MAX_PROCS; i++) {
        if (proc_table[i].pgid == pgid &&
            proc_table[i].state != PROC_UNUSED &&
...
{
    return (sig != SIGCHLD && sig != SIGSTOP && sig != SIGTSTP && sig != SIGCONT);
}
...
uint32_t sched_signal_check(uint32_t frame_esp)
{
    ...
    if (handler == SIG_IGN) {
        ...
        if (is_fatal_default(signum)) {
            ...
            klog_uint("SIGNAL", "default-kill from_crash", (uint32_t)from_crash);
