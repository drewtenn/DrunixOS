...
typedef char core_prstatus_size_check[(sizeof(core_prstatus_t) == 144) ? 1 : -1];
...
typedef char core_prpsinfo_size_check[(sizeof(core_prpsinfo_t) == 124) ? 1 : -1];
...
static int write_exact(uint32_t inode_num, uint32_t offset,
...
static int user_page_info(uint32_t pd_phys, uint32_t vaddr,
                          uint32_t *phys_out, uint32_t *pte_flags_out)
{
    uint32_t *pd;
    uint32_t *pt;
    uint32_t pde;
    ...
    if (phys_out) *phys_out = pte & ~0xFFFu;
    if (pte_flags_out) *pte_flags_out = pte & 0xFFFu;
    return 1;
}
...
static int next_user_segment(process_t *proc, uint32_t search_start,
                             uint32_t *seg_vaddr, uint32_t *seg_size,
                             uint32_t *seg_flags, uint32_t *next_search)
{
...
static uint32_t count_user_segments(process_t *proc)
{
    ...
    while (next_user_segment(proc, search, &seg_vaddr, &seg_size,
                             &seg_flags, &search)) {
        count++;
    }
...
static int write_prstatus_note(uint32_t inode_num, uint32_t offset,
                               process_t *proc, int signum)
{
    static const char note_name[] = "CORE";
...
static int write_prpsinfo_note(uint32_t inode_num, uint32_t offset,
                               process_t *proc)
{
    static const char note_name[] = "CORE";
    static const uint8_t zero_pad[4] = { 0, 0, 0, 0 };
                               ...
                               uint32_t seg_size)
{
    ...
    for (uint32_t off = 0; off < seg_size; off += 0x1000u) {
        if (!user_page_info(proc->pd_phys, seg_vaddr + off, &phys, 0))
            return -1;
        if (write_exact(inode_num, file_offset + off, (const void *)phys, 0x1000u) != 0)
            return -1;
    }
