...
{
    uint32_t access = vma ? (vma->flags & (VMA_FLAG_READ |
                                           VMA_FLAG_WRITE |
                                           VMA_FLAG_EXEC)) : 0;
...
static uint32_t count_present_user_pages(uint32_t pd_phys,
...
static void mem_forensics_emit_map_region(render_buf_t *rb, const struct process *proc,
...
static const char *mem_forensics_fault_name(uint32_t classification)
{
    ...
    case MEM_FORENSICS_FAULT_NONE:       return "none";
    case MEM_FORENSICS_FAULT_UNMAPPED:   return "unmapped";
    case MEM_FORENSICS_FAULT_PROTECTION: return "protection";
    case MEM_FORENSICS_FAULT_COW_WRITE:  return "cow-write";
    case MEM_FORENSICS_FAULT_STACK_LIMIT:return "stack-limit";
    case MEM_FORENSICS_FAULT_LAZY_MISS:  return "lazy-miss";
                                         ...
                                         mem_forensics_report_t *out)
{
    ...
    if ((err & PF_ERR_PRESENT) == 0u) {
        if (vma->kind == VMA_KIND_STACK) {
            uint32_t stack_slack =
                proc->crash.frame.user_esp > 32u ?
                    proc->crash.frame.user_esp - 32u : 0u;
...
static int mem_forensics_append_region(mem_forensics_report_t *out,
                                       uint32_t pd_phys,
                                       uint32_t start, uint32_t end,
                                       uint32_t kind, uint32_t prot_flags,
                                       const char *label)
{
...
int mem_forensics_collect(const struct process *proc, mem_forensics_report_t *out)
{
    if (!proc || !out)
        return -1;
    ...
    if (proc->image_start < proc->image_end) {
        ...
        if (!image_seen) {
            if (mem_forensics_append_region(out, proc->pd_phys,
                                            proc->image_start,
                                            proc->image_end,
                               ...
                               char *buf, uint32_t cap, uint32_t *size_out)
{
    ...
    mem_forensics_emitf(&rb, "Class:\t%s\n",
                              ...
                              char *buf, uint32_t cap, uint32_t *size_out)
{
    ...
    for (uint32_t pdi = 0; pdi < 1024; pdi++) {
        if ((pd[pdi] & (PG_PRESENT | PG_USER)) != (PG_PRESENT | PG_USER)) {
            if (have_region) {
                mem_forensics_emit_map_region(&rb, proc, region_start, region_end,
                                              region_flags);
                have_region = 0;
            }
            continue;
        ...
        for (uint32_t pti = 0; pti < 1024; pti++) {
            ...
            if (vaddr >= USER_STACK_TOP) {
                if (have_region) {
                    mem_forensics_emit_map_region(&rb, proc,
            ...
            if ((pte & (PG_PRESENT | PG_USER)) != (PG_PRESENT | PG_USER)) {
                if (have_region) {
                    mem_forensics_emit_map_region(&rb, proc, region_start,
                                                  region_end, region_flags);
                    have_region = 0;
                }
                continue;
            }
