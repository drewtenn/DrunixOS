...
static void
boot_parse_cmdline(const char *cmdline, int *nodesktop, int *vgatext);
...
static int
boot_framebuffer_grid(const framebuffer_info_t *fb, int *cols, int *rows)
{
	if (!fb || !cols || !rows)
		return 0;
	...
	*cols = fb->cell_cols > FB_CELL_COLS ? FB_CELL_COLS : (int)fb->cell_cols;
	*rows = fb->cell_rows > FB_CELL_ROWS ? FB_CELL_ROWS : (int)fb->cell_rows;
	return 1;
}
...
boot_parse_cmdline(const char *cmdline, int *nodesktop, int *vgatext)
{
	...
	while (*p) {
		...
		if (nodesktop && len == 9 && k_memcmp(tok, "nodesktop", 9) == 0)
			*nodesktop = 1;
		if (vgatext && len == 7 && k_memcmp(tok, "vgatext", 7) == 0)
			*vgatext = 1;
	}
}
...
static void
legacy_console_vga_color(uint8_t color, uint8_t *r, uint8_t *g, uint8_t *b)
{
...
static void
legacy_console_render_cell(int col, int row, unsigned char ch, uint8_t attr)
{
	uint8_t r;
	uint8_t g;
	uint8_t b;
...
static void legacy_console_render_shadow_cell_at_offset(int offset)
{
	...
	legacy_console_render_cell(
	    col, row, (unsigned char)fb_cell->ch, fb_cell->attr);
}
...
static void legacy_console_render_shadow(void)
{
	for (int row = 0; row < console_rows; row++) {
		for (int col = 0; col < console_cols; col++) {
			...
			legacy_console_render_cell(
			    col, row, (unsigned char)cell->ch, cell->attr);
		}
	}
...
static void legacy_console_set_framebuffer(framebuffer_info_t *fb)
{
	...
	cols = fb->cell_cols ? (int)fb->cell_cols : (int)(fb->width / GUI_FONT_W);
	rows = fb->cell_rows ? (int)fb->cell_rows : (int)(fb->height / GUI_FONT_H);
	if (cols <= 0 || rows <= 0) {
	...
	for (int row = 0; row < console_rows; row++) {
		for (int col = 0; col < console_cols; col++) {
			...
			if (row < MAX_ROWS && col < MAX_COLS && old_cell < copy_limit) {
				cell->ch =
				    legacy_console_handoff_char(shadow_vga[row][col * 2]);
...
void start_kernel(uint32_t magic, multiboot_info_t *mbi)
{
	...
	if (mbi && (mbi_flags & MULTIBOOT_FLAG_CMDLINE) && mbi->cmdline)
		boot_parse_cmdline(
		    (const char *)mbi->cmdline, &boot_nodesktop, &boot_vgatext);
...
{
	if (legacy_console_fb) {
		k_memcpy(fb_scrollback[sb_head],
		         boot_fb_cells,
		...
		k_memmove(boot_fb_cells,
		          boot_fb_cells + console_cols,
		...
		k_memcpy((char *)(get_offset(0, 0) + VIDEO_ADDRESS),
...
static void redraw_screen(void)
{
	...
	for (int r = 0; r < console_rows; r++) {
		...
		if (legacy_console_fb) {
			...
			for (int c = 0; c < console_cols; c++)
				legacy_console_render_cell(
				    c, r, (unsigned char)src[c].ch, src[c].attr);
		} else {
			char *src;
			if (logical < sb_count) {
				int idx = (sb_head - sb_count + logical + SCROLLBACK_ROWS) %
...
void set_char_at_video_memory(char character, int offset)
{
	...
	if (legacy_console_fb) {
		...
		legacy_console_render_cell(
		    col, row, (unsigned char)character, current_color);
		return;
	}
