...
#define KTEST_ASSERT_EQ(tc, a, b) \
    do { KTEST_EXPECT_EQ(tc, a, b);       if ((tc)->failed) return; } while (0)

#define KTEST_ASSERT_NE(tc, a, b) \
    do { KTEST_EXPECT_NE(tc, a, b);       if ((tc)->failed) return; } while (0)

#define KTEST_ASSERT_TRUE(tc, expr) \
    do { KTEST_EXPECT_TRUE(tc, expr);     if ((tc)->failed) return; } while (0)

#define KTEST_ASSERT_NOT_NULL(tc, ptr) \
    do { KTEST_EXPECT_NOT_NULL(tc, ptr);  if ((tc)->failed) return; } while (0)
