...
static int mock_root_open(void *ctx,
                          const char *path,
                          uint32_t *inode_out,
                          uint32_t *size_out)
{
...
static int
mock_root_getdents(void *ctx, const char *path, char *buf, uint32_t bufsz)
{
	uint32_t written = 0;
...
static int mock_root_read(void *ctx,
                          uint32_t inode_num,
                          uint32_t offset,
                          uint8_t *buf,
                          uint32_t count)
...
static int mock_sub_open(void *ctx,
                         const char *path,
                         uint32_t *inode_out,
                         uint32_t *size_out)
{
...
static int mock_sub_read(void *ctx,
                         uint32_t inode_num,
                         uint32_t offset,
                         uint8_t *buf,
                         uint32_t count)
...
static int
mock_sub_getdents(void *ctx, const char *path, char *buf, uint32_t bufsz)
{
...
static int
read_procfs_text_file(uint32_t kind, uint32_t pid, char *buf, uint32_t cap)
{
...
static void test_cross_mount_open_prefers_child_mount(ktest_case_t *tc)
{
	...
	KTEST_EXPECT_EQ(
	    tc, (uint32_t)vfs_open_file("mnt/childfile", &ref, &size), 0u);
...
static void test_paths_normalize_like_linux(ktest_case_t *tc)
{
	...
	KTEST_EXPECT_EQ(
	    tc,
	    (uint32_t)vfs_open_file("//mnt/./ignored/../childfile", &ref, &size),
	    0u);
	KTEST_EXPECT_EQ(tc, ref.inode_num, CHILD_FILE_INO);
	KTEST_EXPECT_EQ(tc, size, CHILD_FILE_SIZE);
...
static void
test_sysfs_block_tree_lists_disks_and_partition_metadata(ktest_case_t *tc)
{
	char buf[256];
	char text[64];
	char expected[64];
	...
	KTEST_ASSERT_TRUE(
	    tc, read_vfs_text_path("/sys/block/sda/size", text, sizeof(text)) > 0);
	...
	KTEST_ASSERT_TRUE(
	    tc, read_vfs_text_path("/sys/block/sda/dev", text, sizeof(text)) > 0);
	KTEST_EXPECT_TRUE(tc, k_strcmp(text, "8:0\n") == 0);
	KTEST_ASSERT_TRUE(
	    tc, read_vfs_text_path("/sys/block/sda/type", text, sizeof(text)) > 0);
	KTEST_EXPECT_TRUE(tc, k_strcmp(text, "disk\n") == 0);

	KTEST_ASSERT_TRUE(
	    tc,
	    read_vfs_text_path("/sys/block/sda/sda1/size", text, sizeof(text)) > 0);
	...
	KTEST_ASSERT_TRUE(
	    ...
	    k_snprintf(expected,
	               sizeof(expected),
	               "%u\n",
	               (uint32_t)(DRUNIX_DISK_SECTORS - TEST_PARTITION_START)) > 0);
	KTEST_EXPECT_TRUE(tc, k_strcmp(text, expected) == 0);
	KTEST_ASSERT_TRUE(
	    tc,
	    read_vfs_text_path("/sys/block/sda/sda1/dev", text, sizeof(text)) > 0);
	KTEST_EXPECT_TRUE(tc, k_strcmp(text, "8:1\n") == 0);
	KTEST_ASSERT_TRUE(
	    tc,
	    read_vfs_text_path("/sys/block/sda/sda1/type", text, sizeof(text)) > 0);
...
static void test_proc_mounts_reports_dynamic_sources(ktest_case_t *tc)
{
	...
	KTEST_EXPECT_TRUE(tc,
	                  k_strstr(buf, "devfs /dev devfs rw,nosuid 0 0\n") != 0);
	KTEST_EXPECT_TRUE(
	    tc,
	    k_strstr(buf,
...
static void test_proc_mounts_reports_boot_style_sources(ktest_case_t *tc)
{
	...
	KTEST_ASSERT_EQ(
	    tc, (uint32_t)vfs_mount_with_source("/", "ext3", "/dev/sda1"), 0u);
	KTEST_ASSERT_EQ(
	    tc, (uint32_t)vfs_mount_with_source("/dufs", "dufs", "/dev/sdb1"), 0u);
...
static void
test_proc_vmstat_reports_image_totals_and_region_count(ktest_case_t *tc)
{
	char buf[256];
	int n;
...
static void test_proc_maps_matches_mem_forensics_renderer(ktest_case_t *tc)
{
	...
	n = read_procfs_text_file(
	    PROCFS_FILE_MAPS, 1u, procfs_buf, sizeof(procfs_buf));
	KTEST_ASSERT_TRUE(tc, n > 0);

	KTEST_ASSERT_EQ(tc,
	                (uint32_t)mem_forensics_render_maps(
	                    proc, render_buf, sizeof(render_buf), &rendered),
	                0u);
...
static void test_proc_kmsg_renders_retained_kernel_log(ktest_case_t *tc)
{
	...
	KTEST_EXPECT_EQ(tc,
	                (uint32_t)procfs_file_size(
	                    node.proc_kind, node.proc_pid, node.proc_index, &size),
	                0u);
	...
	offset = (size > (uint32_t)(sizeof(buf) - 1u))
	             ...
	             : 0u;
	n = procfs_read_file(node.proc_kind,
	                     node.proc_pid,
	                     node.proc_index,
	                     offset,
	                     buf,
