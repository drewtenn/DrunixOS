...
static uint32_t map_user_page(process_t *proc, uint32_t virt, uint8_t fill)
{
    ...
    if (paging_map_page(proc->pd_phys, virt, phys,
...
static int init_fork_test_proc(process_t *proc, uint32_t *kstack_words,
                               uint32_t kstack_word_count)
{
    ...
    if (vma_add(proc, proc->image_start, proc->image_end,
...
static void test_copy_to_user_spans_pages(ktest_case_t *tc)
{
    ...
    KTEST_EXPECT_EQ(tc, uaccess_copy_to_user(&proc, start, src, sizeof(src)), 0u);
...
static void test_copy_string_from_user_spans_pages(ktest_case_t *tc)
{
    ...
    KTEST_EXPECT_EQ(tc,
                    uaccess_copy_string_from_user(&proc, buf, sizeof(buf),
                                                  start),
                    0u);
...
static void test_prepare_rejects_kernel_and_unmapped_ranges(ktest_case_t *tc)
{
    ...
    KTEST_EXPECT_EQ(tc,
                    ...
                    (uint32_t)-1);
    KTEST_EXPECT_EQ(tc,
                    uaccess_prepare(&proc, USER_STACK_TOP - 2u, 4u, 0),
                    (uint32_t)-1);
...
static void test_copy_to_user_breaks_cow_clone(ktest_case_t *tc)
{
    ...
    if (uaccess_copy_to_user(&parent, 0x430000u,
                             parent_msg, sizeof(parent_msg)) != 0) {
        KTEST_EXPECT_TRUE(tc, 0);
        destroy_test_proc(&parent);
        return;
    }
    ...
    KTEST_EXPECT_EQ(tc,
                    uaccess_copy_to_user(&child, 0x430000u,
                                         child_msg, sizeof(child_msg)),
                    0u);
    ...
    KTEST_EXPECT_EQ(tc, k_strcmp((char *)mapped_alias(&parent, 0x430000u), parent_msg), 0u);
    KTEST_EXPECT_EQ(tc, k_strcmp((char *)mapped_alias(&child, 0x430000u), child_msg), 0u);

    destroy_test_proc(&child);
    destroy_test_proc(&parent);
}

static void test_release_user_space_drops_only_child_cow_reference(ktest_case_t *tc)
{
    static process_t parent;
    static process_t child;
    uint32_t *parent_pte = 0;
    ...
    KTEST_EXPECT_EQ(tc, k_strcmp((char *)mapped_alias(&parent, 0x440000u), msg), 0u);

    destroy_test_proc(&parent);
}

static void test_process_fork_stack_pages_break_cow_on_write_fault(ktest_case_t *tc)
{
    static process_t parent;
    static process_t child;
    ...
    if (vma_add(&parent, stack_page, USER_STACK_TOP,
                VMA_FLAG_READ | VMA_FLAG_WRITE |
                VMA_FLAG_ANON | VMA_FLAG_PRIVATE | VMA_FLAG_GROWSDOWN,
                VMA_KIND_STACK) != 0) {
        KTEST_EXPECT_TRUE(tc, 0);
    ...
    if (uaccess_copy_to_user(&parent, stack_addr,
                             parent_msg, sizeof(parent_msg)) != 0) {
        KTEST_EXPECT_TRUE(tc, 0);
        destroy_test_proc(&parent);
        return;
    ...
    KTEST_EXPECT_EQ(tc,
                    k_strcmp((char *)mapped_alias(&parent, stack_addr),
                             parent_msg),
                    0u);

    KTEST_EXPECT_EQ(tc,
                    paging_handle_fault(child.pd_phys, stack_addr,
                                        PF_ERR_PRESENT | PF_ERR_WRITE | PF_ERR_USER,
                                        stack_addr + 16u, &child),
                    0u);
    ...
    KTEST_EXPECT_EQ(tc,
                    uaccess_copy_to_user(&child, stack_addr,
                                         child_msg, sizeof(child_msg)),
                    0u);
    KTEST_EXPECT_EQ(tc,
                    k_strcmp((char *)mapped_alias(&parent, stack_addr),
                    ...
                    0u);
    KTEST_EXPECT_EQ(tc,
                    k_strcmp((char *)mapped_alias(&child, stack_addr),
                             child_msg),
                    0u);
...
static void test_process_fork_child_writes_image_data_before_exec(ktest_case_t *tc)
{
    static process_t parent;
    static process_t child;
    static uint32_t parent_kstack_words[64];
    ...
    if (uaccess_copy_to_user(&parent, TEST_BSS_ADDR,
                             parent_msg, sizeof(parent_msg)) != 0) {
        KTEST_EXPECT_TRUE(tc, 0);
        destroy_test_proc(&parent);
        return;
    ...
    KTEST_EXPECT_EQ(tc,
                    uaccess_copy_to_user(&child, TEST_BSS_ADDR,
                                         child_msg, sizeof(child_msg)),
                    0u);
    ...
    KTEST_EXPECT_EQ(tc, pmm_refcount(*child_pte & ~0xFFFu), 1u);
    KTEST_EXPECT_EQ(tc,
                    k_strcmp((char *)mapped_alias(&parent, TEST_BSS_ADDR),
                             parent_msg),
                    0u);
    KTEST_EXPECT_EQ(tc,
                    k_strcmp((char *)mapped_alias(&child, TEST_BSS_ADDR),
                             child_msg),
                    0u);
...
static void test_process_fork_child_survives_parent_exit_and_reuses_last_cow_ref(
    ktest_case_t *tc)
{
    static process_t parent;
    static process_t child;
...
static void test_process_fork_child_stack_growth_is_private(ktest_case_t *tc)
{
    ...
    if (uaccess_copy_to_user(&parent, TEST_STACK_ADDR,
                             parent_msg, sizeof(parent_msg)) != 0) {
        KTEST_EXPECT_TRUE(tc, 0);
        destroy_test_proc(&parent);
        return;
    ...
    KTEST_EXPECT_EQ(tc, uaccess_prepare(&parent, TEST_GROWN_STACK_ADDR, 1u, 1),
                    (uint32_t)-1);
    KTEST_EXPECT_EQ(tc,
                    paging_handle_fault(child.pd_phys, TEST_GROWN_STACK_ADDR,
                                        PF_ERR_WRITE | PF_ERR_USER,
                                        TEST_GROWN_STACK_ADDR + 16u, &child),
                    0u);
    KTEST_EXPECT_EQ(tc, child.stack_low_limit, TEST_GROWN_STACK_PAGE);
    KTEST_EXPECT_EQ(tc, parent.stack_low_limit, TEST_STACK_PAGE);
    KTEST_EXPECT_EQ(tc, uaccess_prepare(&child, TEST_GROWN_STACK_ADDR, 1u, 1),
                    0u);
    KTEST_EXPECT_EQ(tc, uaccess_prepare(&parent, TEST_GROWN_STACK_ADDR, 1u, 1),
                    (uint32_t)-1);
    KTEST_EXPECT_EQ(tc,
                    uaccess_copy_to_user(&child, TEST_GROWN_STACK_ADDR,
                                         child_msg, sizeof(child_msg)),
                    0u);
...
static void test_repeated_fork_exec_cleanup_preserves_parent_refs(ktest_case_t *tc)
{
    ...
    if (uaccess_copy_to_user(&parent, TEST_BSS_ADDR,
                             parent_bss, sizeof(parent_bss)) != 0 ||
        uaccess_copy_to_user(&parent, TEST_STACK_ADDR,
                             parent_stack, sizeof(parent_stack)) != 0) {
        KTEST_EXPECT_TRUE(tc, 0);
        destroy_test_proc(&parent);
        return;
    }
    ...
    for (uint32_t i = 0; i < 32u; i++) {
        ...
        if (paging_walk(parent.pd_phys, TEST_BSS_ADDR, &parent_bss_pte) != 0 ||
            ...
            paging_walk(child.pd_phys, TEST_STACK_ADDR, &child_stack_pte) != 0) {
            KTEST_EXPECT_TRUE(tc, 0);
            destroy_forked_child(&child);
            break;
        }
        ...
        KTEST_EXPECT_EQ(tc,
                        uaccess_copy_to_user(&child, TEST_BSS_ADDR,
                                             child_bss, sizeof(child_bss)),
                        0u);
        KTEST_EXPECT_EQ(tc,
                        uaccess_copy_to_user(&child, TEST_STACK_ADDR,
...
static void test_copy_to_user_handles_three_way_cow_sharing(ktest_case_t *tc)
{
    ...
    if (uaccess_copy_to_user(&parent, 0x450000u,
                             parent_msg, sizeof(parent_msg)) != 0) {
        KTEST_EXPECT_TRUE(tc, 0);
        destroy_test_proc(&parent);
        return;
    ...
    KTEST_EXPECT_EQ(tc,
                    uaccess_copy_to_user(&child2, 0x450000u,
                                         child2_msg, sizeof(child2_msg)),
                    0u);
    ...
    KTEST_EXPECT_EQ(tc,
                    uaccess_copy_to_user(&child1, 0x450000u,
                                         child1_msg, sizeof(child1_msg)),
                    0u);
    ...
    KTEST_EXPECT_EQ(tc, pmm_refcount(*parent_pte & ~0xFFFu), 1u);
    KTEST_EXPECT_EQ(tc,
                    k_strcmp((char *)mapped_alias(&parent, 0x450000u),
                             parent_msg),
                    0u);
    KTEST_EXPECT_EQ(tc,
                    k_strcmp((char *)mapped_alias(&child1, 0x450000u),
                             child1_msg),
                    0u);
    KTEST_EXPECT_EQ(tc,
                    k_strcmp((char *)mapped_alias(&child2, 0x450000u),
                             child2_msg),
                    0u);
...
static void test_process_fork_rolls_back_when_kstack_alloc_fails(ktest_case_t *tc)
{
    static process_t parent;
    static process_t child;
    static uint32_t parent_kstack_words[64];
...
ktest_suite_t *ktest_suite_uaccess(void) { return &suite; }
