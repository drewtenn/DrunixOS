...
static void test_fs_truncate_shrink_then_extend_zero_fills_gap(ktest_case_t *tc)
{
	...
	KTEST_ASSERT_EQ(
	    tc,
	    (uint32_t)fs_write((uint32_t)ino, 0, data, (uint32_t)sizeof(data)),
	    (uint32_t)sizeof(data));

	KTEST_ASSERT_EQ(tc, (uint32_t)fs_truncate((uint32_t)ino, 2u), 0u);
	KTEST_ASSERT_EQ(
	    tc, (uint32_t)fs_truncate((uint32_t)ino, (uint32_t)sizeof(data)), 0u);
...
static void test_fs_mkdir_creates_entry(ktest_case_t *tc)
{
	...
	while (i < n) {
		...
		int j = 0;
		while (want[j] && entry[j] && want[j] == entry[j])
			j++;
		if (want[j] == '\0' && entry[j] == '\0') {
			found = 1;
			break;
		}
		while (i < n && buf[i] != '\0')
