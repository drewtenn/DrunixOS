...
static void run_and_tally(ktest_suite_t *suite, int *total_pass, int *total_fail) {
    int p = 0, f = 0;
...
void ktest_run_all(void) {
     ...
     */
    k_snprintf(summary, sizeof(summary),
               "SUMMARY pass=%d fail=%d", total_pass, total_fail);
