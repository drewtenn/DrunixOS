...
static void test_syscall_fstat_reads_resource_fd_table(ktest_case_t *tc)
{
	...
	KTEST_EXPECT_EQ(tc,
	                syscall_handler(SYS_FSTAT64, 1, 0x00800400u, 0, 0, 0, 0),
	                (uint32_t)-1);
...
static void test_syscall_getcwd_reads_resource_fs_state(ktest_case_t *tc)
{
	...
	KTEST_EXPECT_EQ(
	    tc, syscall_handler(SYS_GETCWD, 0x00800000u, 64, 0, 0, 0, 0), 6u);
...
static void test_syscall_chdir_updates_resource_fs_state(ktest_case_t *tc)
{
	...
	KTEST_ASSERT_EQ(
	    tc, syscall_handler(SYS_CHDIR, 0x00800000u, 0, 0, 0, 0, 0), 0u);
	KTEST_EXPECT_TRUE(tc, k_strcmp(cur->fs_state->cwd, "bin") == 0);
	KTEST_EXPECT_EQ(
	    tc, syscall_handler(SYS_GETCWD, 0x00800100u, 64, 0, 0, 0, 0), 5u);
...
static void test_rt_sigaction_reads_resource_handlers(ktest_case_t *tc)
{
	...
	KTEST_EXPECT_EQ(
	    tc,
	    syscall_handler(SYS_RT_SIGACTION, SIGTERM, 0, 0x00800000u, 8, 0, 0),
	    0u);
...
static void test_linux_syscalls_fill_uname_time_and_fstat64(ktest_case_t *tc)
{
	...
	KTEST_EXPECT_EQ(
	    tc, syscall_handler(SYS_UNAME, 0x00800000u, 0, 0, 0, 0, 0), 0u);
	...
	KTEST_EXPECT_EQ(
	    tc,
	    syscall_handler(SYS_GETTIMEOFDAY, 0x00800200u, 0x00800210u, 0, 0, 0, 0),
	    0u);
	KTEST_EXPECT_EQ(tc, page[0x210], 0u);
	KTEST_EXPECT_EQ(tc, page[0x211], 0u);

	KTEST_EXPECT_EQ(
	    tc, syscall_handler(SYS_FSTAT64, 1, 0x00800400u, 0, 0, 0, 0), 0u);
...
static void test_linux_syscalls_cover_blockdev_fd_path(ktest_case_t *tc)
{
	...
	KTEST_EXPECT_EQ(
	    tc,
	    syscall_handler(SYS_STAT64, 0x00800000u, 0x00800100u, 0, 0, 0, 0),
	    0u);
	...
	KTEST_EXPECT_EQ(
	    tc,
	    syscall_handler(SYS_FSTAT64, (uint32_t)fd, 0x00800180u, 0, 0, 0, 0),
	    0u);
	...
	KTEST_EXPECT_EQ(
	    tc,
	    syscall_handler(
	        SYS_FSTAT64, (uint32_t)fd_part, 0x00800180u, 0, 0, 0, 0),
	    0u);
	...
	KTEST_EXPECT_EQ(
	    tc, syscall_handler(SYS_POLL, 0x00800200u, 1u, 0, 0, 0, 0), 1u);
	KTEST_EXPECT_EQ(
	...
	KTEST_EXPECT_EQ(
	    tc,
	    syscall_handler(SYS_READ, (uint32_t)fd, 0x00800300u, 513u, 0, 0, 0),
	    513u);
	...
	KTEST_EXPECT_EQ(
	    tc,
	    syscall_handler(SYS_READ, (uint32_t)fd, 0x00800300u, 1u, 0, 0, 0),
	    1u);
	...
	KTEST_EXPECT_EQ(
	    tc,
	    syscall_handler(SYS_WRITE, (uint32_t)fd, 0x00800100u, 1u, 0, 0, 0),
	    (uint32_t)-1);
	KTEST_EXPECT_EQ(
	...
	KTEST_EXPECT_EQ(
	    tc,
	    syscall_handler(SYS_LSTAT64, 0x00800000u, 0x00800100u, 0, 0, 0, 0),
	    0u);
	stat_mode = test_read_u32_le(stat_base, 16u);
	KTEST_EXPECT_EQ(tc, stat_mode, 0x0000A1FFu);
	k_strcpy((char *)page, "/sys/block/sda/size");
	sys_fd = (int32_t)syscall_handler(SYS_OPEN, 0x00800000u, 0u, 0, 0, 0, 0);
	...
	KTEST_EXPECT_EQ(
	    tc,
	    syscall_handler(SYS_READ, (uint32_t)sys_fd, 0x00800300u, 5u, 0, 0, 0),
	    5u);
	read_base[5] = '\0';
	...
	KTEST_EXPECT_EQ(
	    tc,
	    syscall_handler(SYS_WRITE, (uint32_t)sys_fd, 0x00800100u, 1u, 0, 0, 0),
	    (uint32_t)-TEST_LINUX_EBADF);
	KTEST_EXPECT_EQ(
	    tc,
	    syscall_handler(SYS_FSTAT64, (uint32_t)sys_fd, 0x00800180u, 0, 0, 0, 0),
	    0u);
	fstat_mode = test_read_u32_le(fstat_base, 16u);
...
test_linux_syscalls_support_busybox_identity_and_rt_sigmask(ktest_case_t *tc)
{
	...
	KTEST_EXPECT_EQ(
	    tc,
	    syscall_handler(
	        SYS_RT_SIGPROCMASK, 0, 0x00800100u, 0x00800200u, 8, 0, 0),
	    0u);
	...
	KTEST_EXPECT_EQ(
	    tc,
	    syscall_handler(
	        SYS_RT_SIGPROCMASK, 1, 0x00800100u, 0x00800200u, 8, 0, 0),
	    0u);
...
static void test_linux_poll_and_select_wait_for_tty_input(ktest_case_t *tc)
{
	...
	((uint32_t *)poll_base)[1] = TEST_LINUX_POLLIN;
	KTEST_EXPECT_EQ(
	    tc, syscall_handler(SYS_POLL, 0x00800100u, 1u, 0, 0, 0, 0), 0u);
	KTEST_EXPECT_EQ(tc, ((uint32_t *)poll_base)[1] >> 16, 0u);

	readfds = (uint32_t *)(page + 0x200u);
	*readfds = 1u;
	KTEST_EXPECT_EQ(
	    tc, syscall_handler(SYS__NEWSELECT, 1u, 0x00800200u, 0, 0, 0, 0), 0u);
	KTEST_EXPECT_EQ(tc, *readfds, 0u);

	tty_input_char(0, 'q');
	((uint32_t *)poll_base)[1] = TEST_LINUX_POLLIN;
	KTEST_EXPECT_EQ(
	    tc, syscall_handler(SYS_POLL, 0x00800100u, 1u, 0, 0, 0, 0), 1u);
...
test_linux_termios_on_stdout_controls_foreground_tty(ktest_case_t *tc)
{
	...
	KTEST_EXPECT_EQ(
	    tc,
	    syscall_handler(SYS_IOCTL, 1u, TEST_LINUX_TCGETS, 0x00800100u, 0, 0, 0),
	    0u);
	...
	KTEST_EXPECT_EQ(
	    tc,
	    syscall_handler(SYS_IOCTL, 1u, TEST_LINUX_TCSETS, 0x00800100u, 0, 0, 0),
	    0u);
...
static void test_linux_syscalls_support_busybox_stdio_helpers(ktest_case_t *tc)
{
	...
	page[0x111] = (uint8_t)'\n';
	KTEST_EXPECT_EQ(
	    tc, syscall_handler(SYS_WRITEV, 1, 0x00800000u, 2, 0, 0, 0), 5u);
	KTEST_EXPECT_EQ(
	    tc, syscall_handler(SYS_WRITE, 1, 0x00800100u, 3, 0, 0, 0), 3u);

	KTEST_EXPECT_EQ(
	    tc, syscall_handler(SYS_GETCWD, 0x00800500u, 32, 0, 0, 0, 0), 2u);
	...
	KTEST_EXPECT_EQ(
	    tc,
	    syscall_handler(
	        SYS_IOCTL, 1, TEST_LINUX_TIOCGWINSZ, 0x00800200u, 0, 0, 0),
	    0u);
	...
	KTEST_EXPECT_EQ(
	    tc, syscall_handler(SYS__LLSEEK, 3, 0, 5, 0x00800300u, 1, 0), 0u);
	...
	u32[0x404 / 4] = 0u;
	KTEST_EXPECT_EQ(
	    tc, syscall_handler(SYS_SENDFILE64, 1, 3, 0x00800400u, 0, 0, 0), 0u);
...
test_linux_open_create_append_preserves_flags_and_data(ktest_case_t *tc)
{
	...
	fd = syscall_handler(
	    SYS_OPEN, 0x00800000u, TEST_LINUX_O_CREAT, 0644u, 0, 0, 0);
	...
	KTEST_EXPECT_EQ(tc,
	                syscall_handler(SYS_WRITE, fd, 0x00800100u, 1u, 0, 0, 0),
	                (uint32_t)-TEST_LINUX_EBADF);
	...
	KTEST_EXPECT_EQ(
	    tc, syscall_handler(SYS_WRITE, fd, 0x00800200u, 1u, 0, 0, 0), 1u);
	KTEST_EXPECT_EQ(tc, syscall_handler(SYS_CLOSE, fd, 0, 0, 0, 0, 0), 0u);
...
static void test_linux_syscalls_install_tls_and_map_mmap2(ktest_case_t *tc)
{
	...
	KTEST_EXPECT_EQ(
	    tc,
	    syscall_handler(SYS_SET_THREAD_AREA, 0x00801000u, 0, 0, 0, 0, 0),
	    0u);
	KTEST_EXPECT_EQ(tc, desc[0], GDT_USER_TLS_ENTRY);
	KTEST_EXPECT_EQ(
	    tc,
	    syscall_handler(SYS_SET_TID_ADDRESS, 0x00801080u, 0, 0, 0, 0, 0),
	    cur->pid);
