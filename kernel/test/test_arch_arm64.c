...
static void test_arm64_vma_add_sorts_and_finds_regions(ktest_case_t *tc)
{
	...
	uint32_t flags = VMA_FLAG_READ | VMA_FLAG_WRITE | VMA_FLAG_ANON |
	                 VMA_FLAG_PRIVATE;
	...
	KTEST_ASSERT_EQ(
	    tc, vma_add(&proc, 0x00408000u, 0x00409000u, flags, VMA_KIND_GENERIC), 0);
	KTEST_ASSERT_EQ(
	    tc, vma_add(&proc, 0x00402000u, 0x00403000u, flags, VMA_KIND_GENERIC), 0);
	KTEST_ASSERT_EQ(
	    tc, vma_add(&proc, 0x00405000u, 0x00406000u, flags, VMA_KIND_GENERIC), 0);
...
static void test_arm64_vma_add_rejects_overlapping_regions(ktest_case_t *tc)
{
	...
	KTEST_EXPECT_EQ(
	    tc, vma_add(&proc, 0x00403000u, 0x00404000u, flags, VMA_KIND_GENERIC), 0);
	KTEST_EXPECT_EQ(
	    tc, vma_add(&proc, 0x00401000u, 0x00402000u, flags, VMA_KIND_GENERIC), 0);
	KTEST_EXPECT_EQ(
	    tc, vma_add(&proc, 0x00401800u, 0x00402800u, flags, VMA_KIND_GENERIC), -1);
...
static void test_arm64_vma_unmap_and_protect_split_generic_mapping(ktest_case_t *tc)
{
	static process_t proc;
	uint32_t flags = VMA_FLAG_READ | VMA_FLAG_WRITE | VMA_FLAG_ANON |
	                 VMA_FLAG_PRIVATE;
	...
	KTEST_ASSERT_EQ(
	    tc, vma_add(&proc, 0x00800000u, 0x00803000u, flags, VMA_KIND_GENERIC), 0);
	...
	KTEST_ASSERT_EQ(
	    tc, vma_add(&proc, 0x00900000u, 0x00903000u, flags, VMA_KIND_GENERIC), 0);
	KTEST_ASSERT_EQ(tc,
...
static void test_arm64_uaccess_copies_mapped_user_bytes(ktest_case_t *tc)
{
	...
	KTEST_EXPECT_EQ(tc,
	                uaccess_copy_from_user(&proc, dst, 0x00400000u, 11u),
	                0);
	KTEST_EXPECT_EQ(tc, (uint32_t)k_strcmp(dst, message), 0u);

	k_memset(dst, 0, sizeof(dst));
	KTEST_EXPECT_EQ(tc,
	                uaccess_copy_string_from_user(&proc,
	                                              dst,
	                                              sizeof(dst),
	                                              0x00400000u),
	                0);
...
static void test_arm64_process_resources_start_with_single_refs(ktest_case_t *tc)
{
	static process_t proc;
