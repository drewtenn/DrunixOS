...
static void module_record_loaded(const char *module_name, uint32_t base, uint32_t size)
{
                     ...
                     uint32_t size)
{
    ...
    if (ehdr.e_type    != ET_REL)  { klog("MOD", "not a relocatable object"); return -1; }
    if (ehdr.e_machine != EM_386)  { klog("MOD", "wrong architecture");       return -1; }
    if (ehdr.e_shnum   == 0)       { klog("MOD", "no sections");               return -1; }
    if (ehdr.e_shnum   > MAX_SECTIONS) { klog("MOD", "too many sections");     return -1; }
    ...
    if (total == 0) { klog("MOD", "no loadable sections"); goto done; }

    /* ---- 4. Allocate load buffer ---- */
    base = (uint8_t *)kmalloc(total);
    if (!base) { klog("MOD", "kmalloc failed"); rc = -3; goto done; }
    ...
    for (uint32_t i = 0; i < shnum; i++) section_addr[i] = 0;
    ...
    if (symtab_idx == 0) { klog("MOD", "no symbol table"); goto done; }

    uint32_t sym_count = shdrs[symtab_idx].sh_size / sizeof(Elf32_Sym);
    if (sym_count > 256) { klog("MOD", "too many symbols"); goto done; }
    ...
    if (!sym_resolved) { rc = -3; goto done; }
    for (uint32_t i = 0; i < sym_count; i++) sym_resolved[i] = 0;
    ...
    for (uint32_t i = 0; i < sym_count; i++) {
        if (syms[i].st_shndx != SHN_UNDEF) {
            ...
            if (syms[i].st_shndx < shnum && section_addr[syms[i].st_shndx] != 0)
                sym_resolved[i] = section_addr[syms[i].st_shndx] + syms[i].st_value;
            else
    ...
    for (uint32_t i = 0; i < shnum; i++) {
        ...
        if (target_sec >= shnum || section_addr[target_sec] == 0) continue;
...
int module_snapshot(module_info_t *out, uint32_t max)
{
    ...
    for (uint32_t i = 0; i < MODULE_MAX_LOADED && count < max; i++) {
        if (!g_loaded_modules[i].loaded)
            continue;
        out[count].loaded = 1;
        k_strncpy(out[count].name, g_loaded_modules[i].name,
