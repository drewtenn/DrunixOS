...
static const char multi_util_source[] = "int tcc_multi_value(void) {\n"
                                        "    return 42;\n"
                                        "}\n";
...
static const char gcc_source[] = "int gcc_value(void) {\n"
                                 "    return 7;\n"
                                 "}\n";
...
static int
run_capture(const char *path, char **argv, char **envp, char *out, int out_cap)
{
	int pipefd[2];
	int pid;
	int used = 0;
...
static int stage_ok(const char *stage,
                    const char *path,
                    char **argv,
                    char **envp,
...
int main(void)
{
	...
	char *runtime_compile_argv[] = {"tcc",
	                                "-nostdlib",
	                                "-static",
	                                "-I/usr/include",
	                                "-o",
	                                "/tmp/tccrt",
	                                "/usr/lib/drunix/crt0.o",
	                                "/tmp/tccrt.c",
	                                "/usr/lib/drunix/libc.a",
	                                0};
	...
	if (stage_ok("TCCCOMPAT: multi compile",
	             "/bin/tcc",
	             multi_compile_argv,
	             envp,
	             0,
	             ...
	             sizeof(out)) != 0)
		goto fail;
	if (stage_ok("TCCCOMPAT: multi run",
	             "/tmp/tccmulti",
	             multi_run_argv,
	             envp,
	             "TCCMULTI OK\n",
	...
	if (stage_ok("TCCCOMPAT: runtime compile",
	             "/bin/tcc",
	             runtime_compile_argv,
	             envp,
	             0,
	             out,
	             sizeof(out)) != 0)
		goto fail;
	if (stage_ok("TCCCOMPAT: runtime run",
	             "/tmp/tccrt",
	             runtime_run_argv,
	             envp,
	             "TCCRT OK\n",
	...
	if (stage_ok("TCCCOMPAT: gcc as",
	             ...
	             sizeof(out)) != 0)
		goto fail;
	if (stage_ok("TCCCOMPAT: gcc compile",
	             "/usr/bin/gcc",
	             gcc_compile_argv,
	             envp,
	             0,
