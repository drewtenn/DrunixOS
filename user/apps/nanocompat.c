...
static int run_capture_input(const char *path,
                             char **argv,
                             char **envp,
                             const char *input,
                             char *out,
...
static int nano_write_ok(
    char **argv, char **envp, const char *input, char *out, int out_cap)
{
...
int main(void)
{
	...
	char *write_argv[] = {
	    "nano", "--saveonexit", "--nohelp", "/tmp/nano-write.txt", 0};
	...
	if (stage_ok("NANOCOMPAT: version",
	             "/bin/nano",
	             version_argv,
	             envp,
	             0,
