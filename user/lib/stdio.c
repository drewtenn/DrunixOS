...
{
	if (!f || !(f->flags & _IO_WRITE) || size == 0 || nmemb == 0)
		return 0;
...
{
	if (!f || !(f->flags & _IO_READ) || size == 0 || nmemb == 0)
		return 0;
	size_t total = size * nmemb;
...
int fclose(FILE *f)
{
	...
	if (f == &_stdin || f == &_stdout || f == &_stderr)
		return 0;
