...
What we do have is a well-known, fixed hardware layout. The BCM2837 in the `raspi3b` model presents 1 GB of RAM from physical address `0x00000000` to `0x3FFFFFFF`. The top 16 MB of that space — from `0x3F000000` to `0x40000000` — is not usable RAM. That region is the **peripheral bus**: memory-mapped registers for the mini-UART, the GPIO controller, and other on-chip devices that ch03 configures to produce output. Writing to those addresses does not write to memory; it programs hardware. Treating them as free pages would be catastrophic.
