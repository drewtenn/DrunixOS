...
- `make test-linux-abi` boots `bin/linuxabi`, a static Linux i386 ELF that uses
  raw `int 0x80` calls and checks 355 Linux-style return values directly.
...
| Processes | `fork`, `vfork`, `execve`, `waitpid`, `wait4`, `exit`, `exit_group`, `kill` | `fork`, `waitpid`, `wait4`, Linux-encoded wait status, and `kill(pid, 0)` probes | BusyBox shell execution and applet subprocesses | `vfork` maps to fork-compatible copy-on-write behavior. |
