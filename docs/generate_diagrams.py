...
# Chapter 7
# Chapter 8
stack(Path('ch08-diag01.svg'),'Physical memory map','Low memory is carved up before the page allocator manages extended RAM.',[('extended RAM','managed by the page allocator','green'),('kernel image + memory bitmap','kernel code, data, and page bitmap','blue'),('video + firmware area','reserved','gray'),('early stack','grows down','amber'),('kernel heap','0x32000-0x8FFFF','green'),('early page maps','identity-mapped paging structures','blue'),('firmware tables','reserved low memory','gray')],h=390)
...
# Chapter 24
stack(Path('ch24-diag01.svg'),'Core dump file layout','An ELF core file begins with metadata, then stores notes and page-aligned memory segments.',[('ELF header','file type is `ET_CORE`','blue'),('Program header table','one `PT_NOTE` header plus `PT_LOAD` headers','green'),('Note payload','`NT_PRSTATUS` and `NT_PRPSINFO` records','amber'),('Alignment padding','next load segment begins on a page boundary','gray'),('Load segments','copied user pages from the crashed process','blue')],h=372)
# Chapter 25
