...
The **ABI** (Application Binary Interface — the register and stack conventions both sides agree on) is the classic 32-bit Linux-style register convention: `EAX` holds the syscall number on entry and the return value on exit; `EBX`, `ECX`, `EDX`, `ESI`, and `EDI` hold up to five arguments. The IDT entry for vector 128 uses a trap gate rather than an interrupt gate, so hardware interrupts remain enabled while `syscall_handler` runs — essential for calls like `SYS_READ` that block waiting for keyboard input to arrive.

### Blocking Calls and Process States

Most syscalls complete immediately, but several must suspend the calling process until an external event occurs. `SYS_READ` on a TTY blocks until input is available. `SYS_WAIT` and `SYS_WAITPID` park the caller until a child exits or stops. Pipe reads and writes block on the shared pipe wait queue when the buffer is empty or full. `SYS_SLEEP` suspends the caller until a future scheduler tick.
...
This is still the same Linux idea: blocking calls are state transitions plus wakeups, not busy loops. A **signal** — an asynchronous notification delivered to a process from the kernel or from another process — can also make a blocked syscall return early. `SYS_SLEEP` reports the remaining whole seconds in that case, matching Linux `sleep(3)` semantics.

### Path Resolution Per Process

Every filesystem-touching syscall — open, create, unlink, mkdir, rmdir, rename, stat, exec, getdents, modload — resolves its path argument against the calling process's **cwd** (current working directory) before passing it to the VFS. The rules are simple: a path starting with `/` is root-relative; an empty cwd means the process is at the root; otherwise the cwd is prepended. This means a process whose cwd is `projects/src` can call `open("main.c")` and the kernel opens `projects/src/main.c` without the user program constructing the full path.
...
`SYS_DUP2` duplicates one fd onto another, closing the destination first if it was already open. The shell uses this in a forked child before calling `SYS_EXEC` to redirect stdin or stdout through a pipe. Because `SYS_EXEC` preserves the calling process's fd table, the exec'd program sees its standard streams already connected to the pipe without any further setup.
...
`SYS_EXEC` replaces the calling process in place. The PID, parent linkage, process-group/session membership, cwd, signal mask, and open-file table survive; the user address space, heap, user stack, entry point, and process metadata derived from argv are rebuilt from the new ELF image. On success the old image never resumes. This is what makes the shell's `fork(); exec(); waitpid();` path genuinely Unix-shaped rather than a spawn wrapper in disguise.
...
| 8  | `SYS_CREATE`        | Create or truncate a file, open writable |
...
| 16 | `SYS_RMDIR`         | Remove an empty directory |
| 17 | `SYS_CHDIR`         | Change the process working directory |
...
| 162| `SYS_SLEEP`         | Suspend the process for N seconds |
| 170| `SYS_MODLOAD`       | Load a relocatable kernel module |
| 171| `SYS_PIPE`          | Create a pipe; return two fds |
| 172| `SYS_DUP2`          | Duplicate one fd onto another |
| 183| `SYS_GETCWD`        | Copy the process cwd into a user buffer |
| 184| `SYS_TCGETATTR`     | Read the TTY termios configuration |
| 185| `SYS_TCSETATTR`     | Write a new TTY termios configuration |
| 186| `SYS_SETPGID`       | Set a process's process group |
| 187| `SYS_GETPGID`       | Return a process's process group |
| 188| `SYS_WAITPID`       | Wait for a PID with WNOHANG/WUNTRACED flags |
| 189| `SYS_TCSETPGRP`     | Set the TTY foreground process group |
| 190| `SYS_TCGETPGRP`     | Get the TTY foreground process group |
| 265| `SYS_CLOCK_GETTIME` | Read a kernel clock into a `timespec` |

### Where the Machine Is by the End of Chapter 16

We now expose a stable ring-3 ABI. Every service user programs depend on — console I/O, process creation and waiting, heap growth, anonymous memory mappings, filesystem operations, pipes, directory enumeration, metadata queries, and signal delivery — is available through a single `int 0x80` entry point. The generic wait-queue model ensures that waiting processes consume no CPU while they sleep, regardless of whether the event source is a TTY, a pipe, a child process, or a timeout. The per-process cwd makes relative paths work naturally, and the per-process VMA table gives the memory-management syscalls a coherent view of heap, stack, and `mmap()` regions.
