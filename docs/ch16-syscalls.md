...
Chapter 15 left us with a ring-3 process running ELF binaries and a context-switch path driven by the PIT. When a user program needs a service that only the kernel can perform — reading a keystroke, writing a file, spawning a child process — it cannot call the kernel directly. Ring-3 code cannot jump into ring-0 memory, and even if it could, the CPU would fault. The only legal crossing point is an arch-specific instruction that the CPU treats as a deliberate, controlled transition from user code to a known kernel entry point. The user program loads a syscall number and arguments into registers, then executes that instruction. The CPU performs a controlled privilege transition — switching to the kernel stack and jumping to the registered handler. The handler saves registers, dispatches to the appropriate kernel routine, writes the return value back into the saved frame, and returns to user code. The mechanism — which instruction triggers it, where the arguments live, what the trap frame looks like — is arch-specific. The dispatch table itself is shared C that both architectures call into once they are safely in the kernel.

![](diagrams/ch16-diag01.svg)

### On x86: int 0x80 and the i386 Trap Frame

On x86, the controlled ring transition is a software interrupt: the user program executes `int 0x80`. The CPU sees that this matches a DPL-3 trap gate in the **IDT** (Interrupt Descriptor Table — the x86 table that maps interrupt and exception vectors to handler entry points) and performs the ring transition. It switches to the kernel stack recorded in the TSS, loads the kernel code segment, and jumps to the ISR stub. The stub saves all general-purpose registers, calls `syscall_handler`, writes the return value back into the saved frame, and executes `iret` to return to ring 3.
...
*On AArch64 (planned, milestone 4): the trap frame on syscall entry preserves `x0`–`x30`, `SP_EL0`, `ELR_EL1`, `SPSR_EL1`.*

**`svc`** (Supervisor Call, the AArch64 instruction class that triggers a synchronous exception to EL1 — the syscall instruction) is the AArch64 equivalent of `int 0x80`. When a user program at EL0 executes `svc #0` — the specific encoding Drunix uses — the CPU raises a synchronous exception, switches to the EL1 stack pointer, and jumps to the vector table entry for the synchronous-from-EL0 slot. No trap gate and no IDT are involved; AArch64 uses a different hardware mechanism from x86, but the effect is the same: a controlled, CPU-enforced transition from unprivileged to privileged code.

The kernel reads **`ESR_EL1`** (Exception Syndrome Register at EL1, the AArch64 register that holds the cause-code and immediate value of an exception — the kernel reads it to distinguish a syscall from a fault) to confirm the exception class is `EC = 0x15` (a Supervisor Call from AArch64 state). The immediate field of `svc #0` is zero by Drunix convention, and the syscall number arrives in `x8` — the register the Linux AArch64 ABI designates for that purpose. Arguments arrive in `x0` through `x5`; the return value is written back into `x0` before the kernel returns.

On syscall entry, the exception entry stub preserves the full general-purpose register file in a trap frame on the kernel stack. The trap frame layout, from the lowest address upward, is:

![](diagrams/ch16-diag02.svg)

The frame captures `x0`–`x30` as 31 consecutive 8-byte slots, then **`SP_EL0`** (Stack Pointer at EL0, the user-mode stack pointer — the kernel saves it so that the user's stack is restored exactly on return) read from the system register of that name, then **`ELR_EL1`** (Exception Link Register at EL1, introduced in Chapter 15 — holds the address of the instruction that was interrupted, so the kernel can return to it after the syscall completes) and **`SPSR_EL1`** (Saved Program Status Register at EL1, introduced in Chapter 15 — captures the processor state flags at exception entry, including the EL0 execution level, so `eret` can restore them). The entry stub writes the syscall return value into the saved `x0` slot in the frame; `eret` then reloads `ELR_EL1` and `SPSR_EL1`, switches back to EL0, and the user program resumes with the return value in `x0`.

Because the dispatch table is plain C — indexed by syscall number, called with a pointer to the saved frame — the same table is used by both arches. Only the entry stub (the few lines of assembly that build the trap frame and call `syscall_handler`) and the return stub differ between x86 and AArch64.
...
We now expose a stable ring-3 ABI whose public surface follows Linux i386 numbering on x86, and whose AArch64 path (planned, milestone 4) will use `svc #0` with the same shared dispatch table. On x86 the entry point is `int 0x80` and arguments arrive in `EAX`, `EBX`, `ECX`, `EDX`, `ESI`, and `EDI`; on AArch64 the entry point is `svc #0`, the syscall number arrives in `x8`, and arguments arrive in `x0`–`x5`. In both cases the dispatcher is plain C, and every service Drunix user programs depend on — console I/O, process creation and waiting, heap growth, anonymous memory mappings, filesystem operations, pipes, directory enumeration, metadata queries, and signal delivery — is available through that single entry point. The generic wait-queue model ensures that waiting processes consume no CPU while they sleep, regardless of whether the event source is a TTY, a pipe, a child process, or a timeout. The per-process cwd makes relative paths work naturally, and the per-process VMA table gives the memory-management syscalls a coherent view of heap, stack, and `mmap()` regions. A tiny static Linux i386 smoke binary, `/bin/linuxhello`, uses Linux syscall numbers directly to exercise that ABI without the Drunix C runtime.
