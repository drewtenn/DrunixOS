...
`paging_create_user_space` allocates a fresh 4 KB page directory from the physical memory manager and copies the kernel's supervisor-only page directory entries into it, including the higher-half aliases. Those copied entries do not carry the `PG_USER` bit, so ring-3 code cannot reach kernel addresses through them even though ring-0 code in the same address space can. That is what keeps a user program from reading or writing kernel memory just because its page directory points at it. The kernel writes page-directory and page-table pages through its kernel-visible aliases rather than relying on user-visible low mappings.
...
5. Copy the segment's on-disk bytes into the newly allocated physical pages through a kernel-visible mapping, without switching `CR3`.
...
The kernel resolves the physical frame backing the top stack page and writes the whole initial stack image through a temporary kernel mapping. No `CR3` reload is required. The same path handles `argc == 0` and `envc == 0` gracefully, writing zero counts and NULL-terminated empty arrays in each case, so the ring-3 startup stub can always pop all three words off the stack without a special case.
...
`/proc/<pid>/maps` walks the process's user page tables, coalesces adjacent present PTEs with the same permissions and VMA label, and prints Linux-like region names: the executable name for the loaded image, `[heap]`, `[stack]`, `anon`, or `file`. Because the maps output is generated from the live page tables, it reflects current resident mappings and current PTE permissions, so permission changes applied with `mprotect()` show up once there is a present page to report. Untouched lazy reservations that exist only in the VMA table, such as a never-faulted anonymous `mmap()` region, are accounted in `/proc/<pid>/vmstat` but do not appear in `maps` until at least one page has actually been materialized. `/proc/<pid>/vmstat` keeps the Drunix-specific totals explicit: reserved bytes come from VMA lengths, mapped bytes come from page-table walks, and committed bytes come from the process address space's committed-page counter. `/proc/<pid>/fd` is a directory whose numbered entries describe the process's open descriptors — DUFS files, pipes, TTYs, stdout, and even synthetic procfs descriptors that were opened earlier — and each `/proc/<pid>/fd/<n>` file renders a one-line description of that fd's current target.
