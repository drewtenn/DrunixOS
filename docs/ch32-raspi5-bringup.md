...
### HVS scanout plane (M9)

M7's HDMI framebuffer worked but the console scroll path crawled visibly down the screen: every newline at the bottom of the screen redrew the framebuffer top-to-bottom in source order, and the HDMI raster caught every intermediate state. The root cause was twofold. First, the firmware framebuffer at `0x3f400000` is mapped Normal-NC (uncached) because `platform_mm_classify` returns `PLATFORM_MM_FRAMEBUFFER` for that range, so every pixel write went straight to DRAM at full latency. Second, the kernel was re-rasterising every glyph cell on every scroll instead of doing the cheap memmove-and-clear pattern Linux's un-accelerated `cfb_copyarea` uses.

The naive fix — keep the firmware framebuffer but ask for a larger virtual buffer with `SET_VIRTUAL_SIZE` so `SET_VIRTUAL_OFFSET` can pan the scanout — does not work. The Pi 5 firmware echoes the requested virtual height in the mailbox response (response bit set, requested value returned verbatim) but `ALLOCATE_BUFFER` only ever allocates the EDID-determined visible plane: empirically tested with PAN_PAGES=2, `virt_h` echoes 2160 but `fb_size = 0x7e9000 = 1920 × 1080 × 4` exactly. There is no buffer past the visible frame to pan into. The legacy mailbox property interface is a backwards-compatibility shim on Pi 5; virtual-buffer hardware scrolling is not part of what it actually implements.

M9 replaces the firmware-managed scanout entirely. The BCM2712 Hardware Video Scaler (HVS6) reads a *display list* (dlist) from on-chip SRAM and composes scanline output for each enabled channel; channel 0 drives HDMI0 on Pi 5. The dlist is a sequence of 32-bit words describing each plane — pixel format, position, dimensions, source buffer base address, pitch. The driver in `kernel/arch/arm64/platform/raspi5/hvs.c` reads the version register (`SCALER6_VERSION` at HVS MMIO offset `0x000`, low byte = chip revision: `0x54` = BCM2712 D0, `0x53` = C0), locates the firmware's primary plane at dlist word 32 (the `HVS_BOOTLOADER_DLIST_END` convention Linux's `vc4_hvs.c` uses), and validates a structural fingerprint against the mailbox-reported framebuffer geometry. If anything's off — wrong dimensions, wrong format, address mismatch, missing end marker — the path fails closed and the console stays on the firmware framebuffer.

When the fingerprint matches, the driver overwrites the plane's address words (dl[5] = high 32 bits, dl[6] = low 32 bits) to point the HVS at a Drunix-owned scanout buffer. The buffer is *not* in BSS — an earlier attempt at this in iter 4 placed a 32 MiB buffer in BSS at link-time and the kernel write-faulted with `ESR_EL1=0x9600004F` (data abort, write permission fault, L3) at `FAR_EL1=0x02000000` as soon as the scroll offset advanced into the `ARM64_INIT_IMAGE_BASE` page range, which the kernel remaps user-RW when `/bin/shell` loads. M9.3 instead introduces a proper PA carve-out: `platform_ram_layout_t` gains `scanout_carve_base`/`scanout_carve_size` fields, raspi5 publishes a 32 MiB block at PA `0x04000000` (above `ARM64_INIT_STACK_TOP = 0x03100000`, well below the firmware framebuffer at `0x3f400000`), and `arch_mm_init` calls `pmm_mark_used` on the range so kheap, ELF loaders, and other PMM clients never touch it. The MMU's `platform_mm_classify` returns `PLATFORM_MM_NORMAL` for the carve-out PAs, so the kernel-linear identity map covers the buffer with Normal-WB Inner-Shareable attributes — cached, fast CPU writes, drained to DRAM with `DC CVAC` before each plane-base advance so the HVS reads consistent data.

The scroll path turns inside out compared to a traditional framebuffer. The carve-out is sized to **four** times the visible frame (32 MiB at 1920×1080×4 = 8 MiB per frame). The HVS scans a 1080-row window starting somewhere inside the 4320-row buffer; `g_hvs_scroll_y` tracks where. On each scroll: advance `g_hvs_scroll_y` by `GUI_FONT_H` pixels, clear one font row at the new bottom of the visible window, `DC CVAC` over that 120 KiB range, update `g_fb_info.address` so subsequent `put_cell` rendering lands at the new visible top, and write the new HVS plane base address into dl[5]/dl[6]. The HVS picks up the new base on next FIFO refill. The CPU does ~120 KiB of work per scroll instead of 8 MiB. When `g_hvs_scroll_y` would advance past `4 × 1080 − 1080 = 3240` rows, the wrap path runs: memcpy the live visible content back to offset 0, CVAC, reset offset. Wrap happens once every `(MULTIPLIER − 1) × visible / GUI_FONT_H = 3 × 1080 / 16 = 201` scrolls.

The volatile dlist word at offset 4 within the plane (PTR0) is **never written** by Drunix. Empirically that word's low 16 bits vary frame-to-frame (observed values `0x80000376`, `0x8000025a`, `0x80000100`, `0x800001ec` across four boots) — the firmware or the HVS itself updates it dynamically with scan position or UPM (Unified Pre-Fetcher) status. A blind write would clobber the prefetcher configuration and break pixel fetching. The address bits live in separate words (dl[5] for high, dl[6] for low); the flip helper writes only those.

Measured scroll cost on hardware drops from ~72 ms per scroll (single-frame buffer, iter 3) to ~484 µs per scroll (virtual scrollback, iter 5) — a 150× speedup. The visible top-to-bottom redraw sweep is gone. `ls` is visually instant. The wrap memcpy still costs ~70 ms once every ~200 scrolls; M9.4 vblank synchronisation via the PixelValve PV0 IRQ on GIC SPI 101 is the planned mitigation, along with replacing the synchronous wrap with a pre-emptive rotation spread across many scrolls.

### Where the Machine Is

By the time the M5 milestone completes its bring-up, the Pi 5 reaches the same in-kernel state the Pi 3 chapter ends in: the kernel image is loaded at `0x80000`, the MMU is enabled with identity-mapped peripheral windows, the GIC-400 is forwarding the generic-timer PPI to CPU 0, the PL011 is producing characters on the chosen debug UART, and the console terminal is polling input. By the time M6 lands, the kernel additionally mounts the ext3 root from the SD card and execs `/bin/shell` — the platform reaches a real userspace shell prompt over serial, the same way virt and raspi3b do today. M7 turns on HDMI: the kernel boot log now mirrors to an attached display, `/dev/fb0` is registered, and `bin/fbfill` paints solid colors to prove the path end to end. M9 hijacks the HVS scanout plane after the mailbox bring-up succeeds, replaces the firmware's framebuffer pointer with a Drunix-owned buffer in cached normal memory, and turns the console scroll path into a one-font-row clear plus an HVS plane-base advance — fast enough that scrolling feels instant. Interactive input on that display still comes in a later milestone.
