...
def main():
    ...
    overlap = sorted(c_set & cxx_set)
    if overlap:
        add_failure(failures, "programs listed in both C_PROGS and CXX_PROGS: " + " ".join(overlap))

    missing = sorted(prog_set - (c_set | cxx_set))
    extra = sorted((c_set | cxx_set) - prog_set)
    if missing:
        add_failure(failures, "programs missing from C_PROGS/CXX_PROGS: " + " ".join(missing))
    if extra:
    ...
    print("userland C and C++ runtime lanes are explicit and complete")
    return 0
